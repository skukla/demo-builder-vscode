...
3. **Generated files must be COMMITTED to the storefront's GitHub repo** — the edge serves from GitHub, not from any local clone. The extension writes to storefront repos via the Git Tree API (`src/features/eds/services/githubFileOperations.ts`); a local `npm install`'s postinstall output is never pushed automatically.
4. **Dropins are a BASE-TEMPLATE concern, not an overlay.** `@dropins/*` ship as one coordinated release set sharing internal chunks (e.g. `tools/chunks/preact-vendor.js`); overlaying current-line dropins onto an older base produces a missing-chunk blank page. This killed the additive "feature pack" mechanism (removed outright; resolution record: `.rptc/complete/2026-06-12-b2b-feature-pack-dropin-delivery.md`). To get B2B dropins, select a package built on `boilerplate-b2b-template` (see `src/features/project-creation/config/demo-packages.json`) — do not bolt b2b dropins onto a non-b2b clone.

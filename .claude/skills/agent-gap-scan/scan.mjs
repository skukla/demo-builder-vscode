...
function readSession(file) {
    ...
    for (const line of readFileSync(file, 'utf8').split('\n')) {
        ...
        for (const c of content) {
            ...
            if (c.type === 'tool_use') {
                calls.push({ id: c.id, name: c.name || '', input: c.input || {} });
    ...
    return { file, calls, results, userTurns };
}
...
let totalCalls = 0, bashCalls = 0, sessions = 0, userTurns = 0;
...
const gapVerbs = Object.keys(OUR_JOBS)
    ...
    .map((v) => ({ verb: v, why: OUR_JOBS[v], ...bashVerbs.get(v) }))
...
const report = {
    ...
    used: ourCalls.map(([t, n]) => ({ tool: t, calls: n })),
};
...
if (opt.json) {
    ...
    for (const [tool, calls] of ourCalls) L.push(`- \`${tool}\` — ${calls}`);
    L.push('');

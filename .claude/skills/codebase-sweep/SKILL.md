...
npx jscpd tests --min-lines 20 --min-tokens 140 --reporters console  # TESTS tree (two floors, below)
npm run health                                                        # all 17 metrics, with the deltas
...
| tests-tree duplication (jscpd) | **TWO FLOORS as of 2026-08-31, because the two populations behave differently.** **SERVICE (.ts): 78 pairs / 3,613 lines** — and ZERO of them is a suite repeating itself; this is duplication BETWEEN files, which is shared setup that wants one home. **WEBVIEW (.tsx): 13 pairs / 1,546 lines** — but 3 pairs and 773 lines of that, half the total, is a suite repeating ITSELF: the same `render(<X ... />)` written out across twenty `it()` blocks, which wants a local `renderX()` helper and carries no design question at all. Same metric, different defect, different remedy. Superseded the single 160 / 2.44% ratchet, which is why that number is not comparable to these. Parameter-bound to `npx jscpd tests --min-lines 20 --min-tokens 140`; the webview/service split is the same run classified by whether either side of the pair is a .tsx file. JITTER: +/-1 between identical runs. `npm run health` records both, with direction, in `reports/health/history.json` | **Each floor may only FALL, independently.** Zero is NOT the target for either: per-suite mock isolation is ratified policy (webview-test-authoring section 2). Judge them apart — a webview rise is usually a new component suite repeating its own render call, a service rise is usually a new family or a copy-pasted preamble. Treating them as one number is what made a 2026-08-31 reading describe the whole tree from its five largest clones, which happened to be .tsx |

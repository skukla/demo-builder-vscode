...
| tests-tree duplication (jscpd) | **160 clones / 2.44%, RATCHET** (measured 2026-08-28 baseline; was 167 before the 2026-08-28 dedup pass). Parameter-bound to `npx jscpd tests --min-lines 20 --min-tokens 140` — a different flag set is a different metric. JITTER: the count moves ±1 between identical runs, so a single-digit change is noise, not movement. COMPOSITION (measured 2026-08-28, 7,863 duplicated lines): **16 clones are a suite repeating ITSELF** (fixable outright, no design question); **41 clones involve a mock-wall suite** (1,658 lines, 21% — these melt with the ADR-015 conversions, no separate work); **103 between other files** (the family-extraction targets: `.rptc/plans/architecture-test-convergence/family-worklist.json` ranks all 89 shared-setup-less families — 20 real targets, 27 small, 42 legitimate splits, ≈2,446 removable lines). Roughly HALF the total is on a worklist with a named mechanism. Reference point: src sits at 0.62% under the same reviewers, so 2.44% is not a law of test code | **The count may only FALL.** Zero is NOT the target: per-suite mock isolation is ratified policy (webview-test-authoring §2) and 42 of the 89 families are legitimate size-splits. The target is an ADJUDICATED FLOOR — every remaining clone either carries a named reason or sits on the worklist. A rise means a new family or a copy-pasted preamble: name it and fix or adjudicate |

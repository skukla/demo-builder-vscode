...
| boundary-cast audit | 49 (6 `as never`, 43 `as unknown as`, 0 `as any`) — measured 2026-08-21 after the full first triage (all 55 sites opened) | any `as any`; any NEW cast; any existing cast on an ARGUMENT to a collaborator (see triage). Remaining sites all carry verdicts: `as never` — 1 documented Spectrum shim (CardActionsMenu), 4 in AddIntegrationFlowAdapter + 1 in stackComponentCollector (both deferred INTO the untyped-channels backlog item). `as unknown as` — ~16 bundled-JSON-to-declared-type consts (config loaders; the declared type is unchecked against the JSON — a real class, but fixing it means schema validation at load, not cast deletion), ~7 synthesized partial HandlerContexts (headlessHandlerContext documents its contract; the command-palette ones are LEADS — each needs its handlers' field reach traced before judging), 4 wizard-state/request launderings in createProjectTool + executor (untyped-channels item), rest local DOM/stream/library shims |

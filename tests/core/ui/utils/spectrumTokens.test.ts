import {
    translateSpectrumToken,
    DimensionValue,
    SpectrumSizeToken,
} from '@/core/ui/utils/spectrumTokens';

/**
 * A string that CLAIMS to be a DimensionValue and is not. The type rejects these
 * at compile time; the runtime still has to hand them back unchanged.
 */
function notAToken(value: string): DimensionValue {
    return value as unknown as DimensionValue;
}
...
describe('translateSpectrumToken', () => {
    ...
    describe('Error Conditions', () => {
        it('should return input unchanged for invalid token', () => {
            expect(translateSpectrumToken(notAToken('size-999'))).toBe('size-999');
        });
        ...
        it('should gracefully handle various invalid inputs', () => {
            ...
            expect(translateSpectrumToken(notAToken(''))).toBe('');
        });
    });
});

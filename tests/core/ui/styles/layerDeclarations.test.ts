...
describe('CSS Layer Declarations', () => {
  ...
  describe('utilities.css', () => {
    ...
    it('has CTA button styles inside overrides layer', () => {
      ...
      assertDefined(layerMatch, 'the overrides layer must be findable in the sheet');
  ...
  describe('wizard.css', () => {
    ...
    it('has .number-badge styles inside theme layer', () => {
      ...
      assertDefined(layerMatch, 'the theme layer must be findable in wizard.css');
      expect(layerMatch[1]).toContain('.number-badge');
    });
  });
});

/**
 * Deployable Selection Model Tests (D2 Track B — Step 01)
 *
 ...
 * to any deployable and mirroring block-libraries' nativeForPackages /
 ...
 * onlyForPackages without coupling the seed deployables.json to packages.
 */

import { getSelectableDeployables } from '@/features/project-creation/services/deployableSelection';
import * as catalogLoader from '@/features/project-creation/services/deployableCatalogLoader';
import type { DeployableCatalogEntry } from '@/types/deployables';
import type { DemoPackage, GitSource } from '@/types/demoPackages';

// getAvailableDeployables is mockable per-test (scoping tests override it). The
...
jest.mock('@/features/project-creation/services/deployableCatalogLoader', () => {
    const actual = jest.requireActual(
        '@/features/project-creation/services/deployableCatalogLoader',
    );
    return {
        ...actual,
        getAvailableDeployables: jest.fn(actual.getAvailableDeployables),
    };
});
...
describe('getSelectableDeployables (real seed catalog)', () => {
    describe('axis filtering (delegates to getAvailableDeployables)', () => {
        it('returns the catalog filtered by backend + frontend', () => {
            const result = getSelectableDeployables(
                makePackage(),
                EDS_PAAS.backend,
                EDS_PAAS.frontend,
            );
...
describe('getSelectableDeployables (package scoping, mocked catalog)', () => {
    const nativeEntry: DeployableCatalogEntry = {
        id: 'native-thing',
        name: 'Native Thing',
    ...
    const restrictedEntry: DeployableCatalogEntry = {
        id: 'buildright-only',
    ...
    const mockGetAvailable = catalogLoader.getAvailableDeployables as jest.Mock;
    ...
    it("marks a nativeForPackages entry as 'required' for that package", () => {
        ...
        const result = getSelectableDeployables(makePackage({ id: 'citisignal' }), 'b', 'f');
    ...
    it("includes an onlyForPackages entry for its OWN package (as 'optional')", () => {
        ...
        const result = getSelectableDeployables(makePackage({ id: 'buildright' }), 'b', 'f');

...
/**
 * The same card, used to CHOOSE rather than to browse.
 *
 * The wizard's sample-data sub-step shows the same grid the panel does — brand
 * art, version picker, data-type count — but there a card is a choice, not a
 * door to a flyout. Rather than a second near-identical card in the wizard, this
 * one grows an optional `selected`.
 *
 * The prop is optional ON PURPOSE, and its presence is the switch: absent, the
 * card stays exactly the `role="button"` the panel has always rendered; present,
...
describe('DatapackCard — as a choice', () => {
    function renderChoice(selected: boolean | undefined) {
        ...
        render(
            <DatapackCard
                group={makeGroup()}
                selectedVersion={makeGroup().versions[0].id.version}
                onVersionChange={jest.fn()}
                onOpen={onOpen}
    ...
    it('stays a plain button for the panel, which does not choose', () => {
        const { card } = renderChoice(undefined);

        expect(card).toHaveAttribute('role', 'button');
        expect(card).not.toHaveAttribute('aria-checked');
    });

    it('becomes a radio once it can be chosen', () => {
        const { card } = renderChoice(false);

        expect(card).toHaveAttribute('role', 'radio');
        expect(card).toHaveAttribute('aria-checked', 'false');
    });

    it('reports itself as the chosen one', () => {
        const { card } = renderChoice(true);

        expect(card).toHaveAttribute('aria-checked', 'true');
    });

    /** Choosing still runs through the same press handler. */
    it('raises the press when chosen, as the browsing card does', () => {
        const { card, onOpen } = renderChoice(false);

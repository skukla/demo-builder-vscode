/**
 * DaLiveContentOperations — deleteAllSiteContent.
 *
 * What remains of `daLiveContentOperations-utils.test.ts` after
 * `filterProductOverlays` moved to `daLiveContentCopy-filterProductOverlays.test.ts`
 * on 2026-09-07 (PL-45). `deleteAllSiteContent` is a method on the class declared
 * here, so these tests were the ones already scored against the right module.
 */

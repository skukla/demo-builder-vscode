/**
 * AppBuilderComponentsList Component Tests (D2 Track B — Step 05)
 *
 * The separate "integrations" surface on the dashboard. Renders one
 * AppBuilderComponentRow per kind:'integration' entry in the project (the mesh keeps its
 * own badge and is EXCLUDED here). Exposes an "Add an App Builder component" affordance that
 * opens the stack-filtered catalog picker plus a custom GitHub-URL door; a
 * catalog choice posts addAppBuilderComponent {id}, a custom URL posts addAppBuilderComponent
 * {source:{owner,repo}}.
 *
...
function projectWith(appBuilderComponents: Project['appBuilderComponents']): Project {
    return {
        name: 'demo',
        path: '/p',
...
describe('AppBuilderComponentsList', () => {
    ...
    describe('remove confirmation guard', () => {
        ...
        it('opens a confirm dialog on Remove WITHOUT posting removeAppBuilderComponent directly', async () => {
            const user = userEvent.setup({ advanceTimers: jest.advanceTimersByTime });
            render(<AppBuilderComponentsList project={deployedProject()} catalog={CATALOG} />);
    ...
    describe('add-a-appBuilderComponent picker', () => {

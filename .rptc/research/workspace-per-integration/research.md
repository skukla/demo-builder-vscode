...
| Commerce profile | ~~`apiServiceResolution.ts` `toServiceSubscriptionInfo`~~ | **Already built, and the citation was dead.** `profileForTenant` (`subscriptionList.ts:67`) matches the project's configured tenant; the named function does not exist in `src/` and is what made this read as outstanding. Checked 2026-09-20. Nothing to do here. |

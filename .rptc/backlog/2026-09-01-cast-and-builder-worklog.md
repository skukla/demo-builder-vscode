...
- 2026-09-08  SECOND occurrence the same day, and this one had a COST: the flake fired inside the pre-push hook on 2026-09-08 and REFUSED a push whose tree was green (the very next 'npm run gate' passed 1559/1559, then the retried push passed). That moves it from an observation to the failure mode PL-41 exists to prevent — a gate failing for a reason unrelated to the change teaches people to re-run rather than read. Two hits in one session across rule-proofs; the common factor is hook-proof shell scripts under full parallel load.

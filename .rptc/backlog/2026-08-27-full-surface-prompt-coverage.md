...
- 2026-08-28  2026-08-27 evening loop: 31 skill-coverage prompts authored (every baseline entry in both scopes now has its probe; validated by coverage test + 17/17 scorer self-tests); skills-baseline _gate updated — v27 verified ON DISK for both bundles, the F5 edge for MEASUREMENT is gone. Tier-2 harness taken to the design gate (battery/tier2-design.md): the secrets-in-export finding (createExportSettings(…, true)) makes the current-project flip a HARNESS step, never an agent step; confirm-gated tools excluded outright; implementation deliberately deferred to a live-host session because none of its safety mechanics could be verified against a dead socket. select_* readOnlyHint queued as an owner decision — deliberate comment on one side, measurement isolation on the other. ENV: dev host died during the 20:05 post-merge reload (ECONNREFUSED, no process); every battery RUN waits on F5.
- 2026-08-27  feat(battery): 31 skill-coverage prompts — every promised skill has its probe (`4323836ea`)
- 2026-08-28  2026-08-28 (post-F5 continuation — the owner pointed out the loop CAN launch the dev host; the working recipe is the app binary + short --user-data-dir + --extensionDevelopmentPath, now recorded because Cursor's CLI shim silently drops the dev flag and a long data-dir path breaks its own IPC socket): SKILLS BASELINE AT ZERO. 34/34 skillHit across calibration + home (14/14) + project (17/17) batches. Both bundle writers verified on v27; the open question is ANSWERED — headless runs register and invoke project skills. One of the item's three done-conditions is met (skills baseline zero for the current shape); remaining: tool baseline to tier-3-only (needs the tier-2 harness, design ready), and the release-cut full sweep.
- 2026-08-28  2026-08-28 tier 2 LIVE: harness built (tier2-harness.mjs + tier2-writes.txt + scratch fixture derived from bodea's real manifest, all values faked), self-tested, negative control green (tier-2 prompts invisible without --tier2), and the first write batch ran 5/5 HIT with the restore VERIFIED (prior pointer bodea → scratch → bodea; scratch reset; repo clean). Baseline 59→54 (the five prompted writes left). FINDINGS: (1) 3 of 5 writes scored TOOL-INSUFFICIENT — the agent called our tool then ls'ed to VERIFY, evidence our write responses don't confirm enough to be trusted; (2) the export agent ls'ed bodea while current=scratch — cwd vs current-project disagreement confuses agents; (3) my own restore banner matched 'RED' inside 'RESTORED' (substring bug, fixed same turn — the self-test-your-instrument lesson again). One wrong assumption corrected: the isolated instance DID have a current project; the choreography absorbed it.
- 2026-08-28  2026-08-28 both tier-2 findings turned into shipped work (owner-directed). FINDING 2 → connection scoping: the proxy sends a #cwd preamble (unambiguous vs MCP's '{' first byte; bare clients untouched); the server resolves it to the containing project and scopes that CONNECTION — reads load the session's project fresh, saves route to saveProjectConfigOnly so a scoped session can never flip the dashboard pointer; get_current_project answers scope: session-directory|dashboard-pointer; AGENTS.md v28 states the promise. Live-proven three ways: units (14), direct socket, and the production proxy from bodea's cwd (scope=session-directory, project=Bodea) with the bare probe still pointer-scoped. FINDING 1 → the three shell-verified writes now NAME their confirming read and say WHERE state lives (extension storage/manifest — where ls can never see it): set_project_pinned, save_ai_prompt, export_project_settings. One real bug found by the new tests' first run: the preamble sniff paused the socket and the MCP transport never resumes an explicitly-paused stream — nine live-socket suites hung until resume moved after transport attach.
- 2026-08-28  2026-08-28 closeout loop: all three remaining pieces done. Tier 2 grew 5→8 (configure_project, regenerate_ai_files, rename_integration — each handler-read; set_project_destination REJECTED by its read: the migration deploys, tier 3 despite the local-sounding name; fixture seeds a real-shaped integration entry). The 51-entry baseline is now THE NAMED FLOOR — every entry carries its reason (cloud/confirm-gated/UI/machine/live-backend/session/harness), enforced both directions by toolPromptCoverage. sweep.sh is the release-cut full-coverage entry, listed in cut-release. Live wave-3: regenerate + rename HIT clean; rename's first run exposed a RIG BUG (bare vs prefixed --allowed-tools names → FLAKY denials; fixed, re-run clean) plus a behavioral note: the harness denial text invites 'other tools' and the agent hand-edited the manifest on cue. STANDING FINDING (reproduced 2/2): for 'set a config value' agents route to raw update_project_config over configure_project — the raw door bypasses secret-routing and staleness-marking; whether to narrow/retire the raw tool is a tool-verdicts decision for the owner. All three done-conditions met.
- 2026-08-28  2026-08-28 narrow-the-raw-tool (owner decision): the routing finding's root cause was OUR OWN GUIDANCE — five skill templates instructed update_project_config for config values (written before configure_project existed; two showed a key/value signature the raw tool never had). Re-pointed all five to configure_project with its real env syntax; the raw tool's description now routes value-setting away from itself and keeps its legitimate jobs (manifest, whole-file .env); v29 bump so existing projects' skills correct on the next sweep. MEASURED: t2-configure went 0/2 before → HIT with a clean 4-call route after the description change alone.

...
#### Webview payloads are typed, then cast away ([`2026-08-20-webview-payloads-are-typed-then-cast-away.md`](2026-08-20-webview-payloads-are-typed-then-cast-away.md))

**Measured, and the count is the point: five of seven producers declare a payload type, and not one reaches the component that reads it.** `getInitialData` is abstract as `Promise<unknown>` (`baseWebviewCommand.ts:166`), each named type — `ConfigureInitialData`, `AiOverviewInitialData`, `InitialWizardData` — appears in exactly ONE file, its producer, and the cast at `baseWebviewCommand.ts:213` widens whatever was declared to `MessagePayload` one line after it is produced. So producer and consumer agree by convention, and a declared type is worse than `Record<string, unknown>` here because it LOOKS checked. Same shape as the `stackBackend` casts CLAUDE.md records, milder blast radius. This is the gap the project-title drift came through: `getProjectDisplayName` existed and was still missed four times, and the brand added in `95bdf0b0` cannot cross this boundary. **ATTEMPTED 2026-08-20 AND REVERTED — for what it found, not for anything that went wrong.** Wiring producer and consumer to one declaration surfaced TEN disagreements that had never been compared, and was still finding more rather than converging when it stopped. Two are bugs today and need none of the refactor: `isWizardStep` validates two of the four fields the consumer needs, and `wizardHelpers` filters on `enabled` in three places — so a config that lost that field passes validation and every step silently disappears; and the Configure payload's `envVars` carry no `key` although `ComponentEnvVar` requires one (`UniqueField` happens to rebuild it, which is why nothing has broken). Four more are duplicate or colliding declarations — the wizard-step shape is declared FOUR times and `WizardStep` names two unrelated things — which have to be resolved BEFORE the payload module can import an unambiguous name. So the plan now starts at Step 0, and the honest estimate is a scoped session, not a mechanical pass. Filed 2026-08-20.
...
#### What does a datapack removal actually delete? ([`2026-08-17-what-does-a-datapack-removal-actually-delete.md`](2026-08-17-what-does-a-datapack-removal-actually-delete.md))
...
#### Storefronts are hardcoded to `main` ([`2026-08-20-storefront-branch-is-hardcoded-main.md`](2026-08-20-storefront-branch-is-hardcoded-main.md))

**CLOSED 2026-08-20, on scope.** An existing repo defaulting to `master` is refused; should we instead build on whatever branch it already uses? The service layer is ALREADY parameterised (~12 methods take `branch`), so this is threading, not redesign — the hardcoding is ~19 call sites plus seven `main--{repo}--{owner}` URL builders, all listed in the item. **Declined on scope, not on a platform rule:** this tool builds throwaway demo storefronts from a `main` boilerplate, so a storefront already living on `master` does not arise. Whether Adobe serves `.aem.live` from a non-`main` ref is still UNRESOLVED and deliberately not what the decision rests on — their "always use the main branch" quote is ambiguous and was over-read here once already. Also records a correction: a non-`main` branch was NOT why Helix 404'd on the original report — `admin.hlx.page` 401s for a known site on any ref, including a nonsense one, so the ref plays no part in site lookup. Filed and closed 2026-08-20.

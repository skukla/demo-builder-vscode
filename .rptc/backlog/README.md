...
#### AI-surface coverage: can an agent actually USE the extension's features? ([`2026-08-16-mcp-surface-for-sc-design-work.md`](2026-08-16-mcp-surface-for-sc-design-work.md))

**58 MCP tools, zero for authoring a page and zero for design.** The surface is optimized for PROVISIONING a project, not for BUILDING the demo inside it — which for EDS is the wrong gap, since the storefront largely IS its content. Evidenced by a full Bodea build session: ~20 DA.live pages authored, nav rebuilt, 28 block doc pages generated, link closure checked — **every operation done by hand with `curl` against `admin.da.live` and a hand-rolled IMS token**, because none is reachable through the MCP. An agent can `list_dalive_sites` and `cleanup_dalive_site` but cannot write a page to one. **The work is smaller than it looks:** 5 of the 6 proposed transport tools just expose existing service methods (`createSource`, `listDirectory`, `deleteSource`, `previewAndPublishPage`, `unpublishPage`); only `read_page` has nothing behind it. **The token win is concentrated in one knowledge tool** — `get_block_authoring_shape` returns the `plugins.da` row already sitting in `component-definition.json`, replacing the ~121,000 tokens a subagent spent deriving 8 blocks' shapes from their JS with roughly 200. Three further knowledge items fold into existing tools rather than becoming new ones. Composites nearly vanished under scrutiny: only write→publish is invariant, and it belongs as a flag. A fourth class emerged unplanned — **verification** — since this session's most valuable checks (link closure across plain text as well as `href`, which found 14 dead paths an href-only crawl structurally could not see) were all hand-rolled. Five traps recorded for the skill layer. Three questions open: per-task tool scoping, whether an open-ended design skill should exist, and whether `write_page` should validate markup (deliberately deferred until knowledge tools show whether agents still get it wrong once they can look the shape up). **The agent layer does not exist:** a generated project ships no `.claude/agents/` at all, and the seven `appbuilder-*` entries that look like agents are SKILLS with persona names. That exposes an asymmetry — App Builder gets seven role-shaped skills (a team), while the EDS storefront, which is what an SC actually builds, gets fourteen task-shaped skills, zero roles and nothing for design. Filed 2026-08-16.

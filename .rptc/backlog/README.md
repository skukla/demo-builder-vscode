...
### Logs toggle → sidebar utility ([`2026-06-12-logs-toggle-to-sidebar.md`](2026-06-12-logs-toggle-to-sidebar.md))

Consolidate the "Logs" affordance from three button locations (dashboard ActionGrid + the generic wizard footer + ProjectCreationStep's three footer states) into a single **Logs utility in the sidebar UtilityBar** (4th icon beside Tools/Help/Settings). **Owner decisions locked:** keep the **toggle** behavior (the sidebar handler reuses the existing `toggleLogsPanel()` chokepoint — no rewrite to one-shot open) and consolidate **fully** (remove the wizard footer Logs buttons too). Also deletes the dead `viewDebugLogs` handler. Research confirmed the sidebar works during the wizard (reveal is guarded by `!hasOpenWebview()`), and the "smart toggle remembers last channel" behavior was never implemented (don't build it). One deliberate call for the implementer: the `resetToggleStates` desync now that the toggle lives in the always-present sidebar. Research base: `/rptc:research` 2026-06-12. Implementation-ready as a `/rptc:feat`.

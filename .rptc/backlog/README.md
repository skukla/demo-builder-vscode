...
**OPEN, blocked on one question — do not close it on the doc quote.** An existing repo defaulting to `master` is refused; should we instead build on whatever branch it already uses? The service layer is ALREADY parameterised (~12 methods take `branch`), so this is threading, not redesign — the hardcoding is ~19 call sites plus seven `main--{repo}--{owner}` URL builders, all listed in the item. **The blocker:** does Adobe serve `.aem.live` from a non-`main` ref in production? Their go-live checklist says *"Always use the main branch for production sites"*, which reads equally as "a branch named main" or "your primary branch" — the docs say nothing about branch NAMING, and that quote was briefly written up as settling it before the ambiguity was spotted. Settle it by testing `admin.hlx.page/status/{owner}/{repo}/master` against a real master-branch Helix site, or by asking the Code Sync team. Keep the current guard meanwhile: our URL builders emit `main--` regardless, so a `master` repo cannot work today whatever Adobe means. Filed 2026-08-20.

...
### Drop-in version coupling — citisignal drifted, buildright unexpectedly empty ([`2026-06-09-dropin-version-coupling.md`](2026-06-09-dropin-version-coupling.md))

`citisignal-eds-boilerplate` `@dropins/storefront-recommendations` is a **full major version** behind canonical (3.x vs 4.x); most other `@dropins/*` are 1-3 minors behind. `buildright-eds` doesn't pin any `@dropins/*` at all despite being a Commerce storefront — needs architectural clarification. Recommendation: don't bump versions unilaterally; treat as input to the thin-layer evaluation (if the forks are retired, both findings resolve). Filed as input data, not a standalone action.

### PDP graceful "Product not available" empty state ([`2026-06-09-pdp-graceful-empty-state.md`](2026-06-09-pdp-graceful-empty-state.md))

Deferred during BYOM PDP routing Phase 1. When an SC deletes a SKU, the cached PDP URL still serves the template and the drop-in queries Commerce, gets nothing back. Cleanup tooling (Refresh PDPs action, action-side telemetry) was rejected as overkill — graceful empty-state handling in the drop-in covers the same degradation more cleanly and applies to any "Commerce-returns-nothing" scenario, not just deletion. **Investigate first**: `@dropins/storefront-pdp` likely has a built-in or configurable empty state; building a wrapper without checking is the wrong order. Phase 0 investigation 15–30 min, implementation 0–2 h depending on what we find. Lives in storefront-template code OR as a Demo Builder code patch (depends on thin-layer evaluation outcome). Cross-references the drop-in version coupling finding — citisignal's drift may affect the empty-state detection API.

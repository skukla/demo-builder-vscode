...
### Commerce-connect for AEM-SC two-instance demos ([`commerce-connect-aem-sc/`](commerce-connect-aem-sc/overview.md))

Extend the extension to **AEM SCs**: an AEM SC runs their own copy, pastes the Commerce SC's **published storefront URL**, **discovers** the (already-public) commerce connection, **scaffolds** an AEM-authorable `aem-boilerplate-xcom` storefront in their own org, **applies** the connection, and authors it in Universal Editor against their own AEM Sites — sharing the commerce **backend** (a URL + public read keys), not code or content. Federated and discovery-first: each SC runs a single-org instance, **no multi-IMS refactor**. Designed (research/decision trail in git history); paused. Start the [`overview.md`](commerce-connect-aem-sc/overview.md), which maps the lead design (connection kit), operator model (federated), front-door/first-run design, the 5-slice [roadmap](commerce-connect-aem-sc/roadmap.md), and the TDD-ready [Slice 1](commerce-connect-aem-sc/slice1-discovery.md) (a pure discovery service). Architecture seam: [ADR-003](../../docs/architecture/adr/003-multisite-architecture-seam.md).

...
#### Content patches have no drift gate ([`2026-08-23-content-patches-have-no-drift-gate.md`](2026-08-23-content-patches-have-no-drift-gate.md))

Filed 2026-08-23: the LKG gate verifies CODE ledgers against clonable canonicals; content patches target DA.live pages it never reads, so two obsolete content patches sat undetected until a user read the create/reset toast (both retired same day — the CitiSignal source had absorbed their fixes). Three fix shapes in the item: probe the PUBLISHED mirror pages on a schedule, count consecutive non-applies from the patch report, or accept the toast as the detector (its copy now asks for a report instead of promising the gate).

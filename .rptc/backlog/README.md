...
#### EDS reset times out on a large template ([`eds-reset-tree-too-large.md`](eds-reset-tree-too-large.md))

**Breaks reset outright, every time, for anything on a big template.** `resetRepoToTemplate` downloads the template zipball then sends ONE `create-tree` with every file's content inline (`githubFileOperations.ts:600-645`, no batching) — measured live on `boilerplate-b2b-template`: 4.74 MB archive, 3,342 files, "Creating tree with 3344 entries", GitHub times out ~12s later telling us to build the tree incrementally. Not a regression; B2B is just the first template big enough to cross the line, and templates only grow. The fix is half-written: `createTree` already accepts a `baseTree` and forwards it as `base_tree` — the reset simply never passes one, while `blockCollectionHelpers.ts:273` already does. Chunk and chain. Batch size wants measuring, since the ceiling is request BYTES not entry count. Filed 2026-08-15.

...
### Unify DA.live site name with GitHub repo name — Phase 1 ([`2026-06-08-unify-da-site-and-repo-name.md`](2026-06-08-unify-da-site-and-repo-name.md))

Make new EDS storefronts always satisfy `daLiveSite === repoName` by removing the separate DA.live site name input from the wizard. Eliminates the bug class behind two fixes already shipped this session (commits `164fd251` and `85a7f288`) — both were necessary because the dual-identifier model leaks into every layer that touches Configuration Service registration, URL stamping, or publish lookups. New projects collapse to one name; existing projects keep working via the legacy-cleanup path that already shipped. Single-day work; pairs with [Phase 2](2026-06-08-rename-existing-da-content-to-repo-name.md), which closes the loop on existing projects.

### Rename existing DA.live content to match repo name — Phase 2 ([`2026-06-08-rename-existing-da-content-to-repo-name.md`](2026-06-08-rename-existing-da-content-to-repo-name.md))

One-time DA.live content rename for existing pre-Phase-1 storefronts: copy DA files from `<daLiveOrg>/<daLiveSite>` to `<daLiveOrg>/<repoName>`, update the Configuration Service registration, update the local manifest, delete the old DA site. Destructive; requires user consent per project. Once shipped across the fleet, retires the `legacyLookupKey` infrastructure and collapses `buildSiteConfigParams` from four args to two. Multi-batch (~3–5 days); pick up after Phase 1 has been in the wild long enough that pre-Phase-1 storefronts are rare.

...
#### parseJSON's 33 call sites are each an unchecked cast ([`2026-08-21-parsejson-call-sites-are-unchecked-casts.md`](2026-08-21-parsejson-call-sites-are-unchecked-casts.md))

**Fan-in × weak contract, the repo's largest instance:** `parseJSON<T>` parses and casts — the generic is a promise nobody verifies — and 33 call sites inherit that, from registry loads to `aio --json` output to service responses. Three are now check-backed (manifest, wizard-steps, bundled configs); the rest trust their writer. Per-site triage bucketed by WRITER (build-shipped = fine; our-own-writes = schema-generate where silent failure hurts; external-process output = the high-value class, drift guaranteed eventually). Explicit constraint: no global throw-on-mismatch — 33 sites have 33 failure-tolerance policies. Filed 2026-08-21 from the call-site-signals discussion.

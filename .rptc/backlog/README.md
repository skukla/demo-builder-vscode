...
#### Bundled config JSON is cast to its declared type, never validated ([`2026-08-21-bundled-config-json-is-cast-not-validated.md`](2026-08-21-bundled-config-json-is-cast-not-validated.md))

**~16 `as unknown as` casts on imported config JSON, and a probe proved they are load-bearing:** `resolveJsonModule` can't produce literal unions (`type: string` vs the interface's union), so tsc structurally CANNOT check these imports — the suppressed check is only recoverable at runtime. Every bundled config (components, demo-packages, stacks, block-libraries, app-builder catalog) can drift from its interface with no failure until a feature reads the drifted field; the wizard-steps `enabled` bug and the mocked-vs-bundled-JSON test trap are this class. Fix shape decided per file: a Zod config-contract test suite (cheap, CI-time) or validate-at-load through `ConfigurationLoader`; derive schema and interface from one source. Expect the first validation run to surface existing mismatches. Filed 2026-08-21 from the boundary-cast audit.

#### Partial HandlerContexts are cast to full and handed to handlers ([`2026-08-21-partial-handler-contexts-cast-to-full.md`](2026-08-21-partial-handler-contexts-cast-to-full.md))

**Seven non-panel entry points build a few-field `HandlerContext` and cast it to the full ~15-field interface** — the shape behind historical "undefined manager" failures, and the reason `createPanelHandlerContext` exists for panels. Each site needs its own reachability trace (which handlers, which `context.<field>` reads) before a verdict: narrow the callee's parameter to what it uses (cast dies, compiler enforces), fix a found gap as a bug, or keep the one documented deliberate partial (`headlessHandlerContext` — whose safety currently rests on prose; the trace could turn it into evidence). Seven self-contained slices. Filed 2026-08-21 from the boundary-cast audit.

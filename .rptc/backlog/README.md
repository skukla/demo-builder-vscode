...
#### Instance wipe option — remove as much data as the service allows ([`2026-08-22-instance-wipe-option.md`](2026-08-22-instance-wipe-option.md))

**Successor to the answered removal question.** A datapack removal is pack-scoped and cannot clear an instance (service owner, 2026-08-22 — hand-created data is out of its reach), so the "clean slate" want becomes its own feature: an explicit wipe action that removes as much as the service permits, honest about its floor. Four design questions in dependency order: what the maximal client-side wipe consists of (union of known packs' data types); whether the service can enumerate imports beyond this project's own pack; whether the service should grow a TRUE wipe (a feature request that may obsolete the client-side union — ask early); and the surface (Data Installer action / dashboard kebab / MCP tool — `reset_datapack` may already be the per-pack primitive). Constraints: never promise "clean instance" from the client side, and surface duration honestly (a six-type removal measured 470s, so a multi-pack wipe is long). Filed 2026-08-22.

...
### B2B feature pack — dropins never reach the browser ([`2026-06-12-b2b-feature-pack-dropin-delivery.md`](2026-06-12-b2b-feature-pack-dropin-delivery.md))

**HIGH — owner needs B2B today. Research-first.** The b2b feature pack adds `package.json` deps + blocks + initializers but never delivers the dropins' runtime code: EDS loads dropins via an import map → vendored `scripts/__dropins__/<dropin>/` files, and the feature pack vendors neither those files nor the import-map entries (`featurePackInstaller.ts` + `FeaturePack` schema have no path for it). Evidence on `citisignal-b2b`: 0/6 b2b `__dropins__`, no b2b import-map entries, 1/6 initializers, stale 404 dep names. The canonical delivery is the storefront's `postinstall` build (`build.mjs` vendors `@dropins/*` from node_modules into `__dropins__` + import map), whose output must then be committed to the storefront GitHub repo (Helix serves from GitHub). Distinct from the shipped MCP isolated-install fix. Includes a manual stopgap for the existing storefront. Research questions + kickoff in the file.

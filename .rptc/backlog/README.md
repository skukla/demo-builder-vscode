...
3. **Package-bound — ⚙️ MECHANISM LANDED, DORMANT** ([`2026-06-17-appbuilder-app-package-bound.md`](2026-06-17-appbuilder-app-package-bound.md)). The `nativeForPackages`/`onlyForPackages` binding mechanism shipped alongside slice 2 — `types/appBuilderComponents.ts` carries the fields and `appBuilderComponentSelection.ts` implements the scoping (`onlyForPackages` excludes; `nativeForPackages` → auto-included/locked). NOT activated: no catalog entry binds a package, and `app-builder-components.schema.json` doesn't yet declare the two fields. Remaining: add the fields to the schema, bind an entry (e.g. the headless mesh → citisignal-headless package) in the JSON, verify the auto-included/locked display in the review surface. Small.

...
### Mesh deployment state: one fact, five readers, two writers — ✅ SHIPPED 2026-08-04 ([`../complete/2026-08-04-mesh-deployment-state-one-accessor.md`](../complete/2026-08-04-mesh-deployment-state-one-accessor.md))

Closed, with two of its four questions found already solved — the item over-counted by treating each same-day fix site as standing duplication. Genuinely fixed: the second writer (`deployMeshHeadless`'s hand-set instance status was redundant with `recordDeployOutcome` inside `updateMeshState`; the `deploying` write stays, since that one has no other writer), and the third encoding of "stale or update-declined" (now `isUpdatePending` in `statusVocabulary`). The collapse also exposed a mock validating the pre-ADR-011-D3 model — three assertions in `deployMesh-storage` passed only because the redundant writer masked an unfaithful double. **Reopened and re-fixed the same day:** the closure compared readers and status writes but never what the two deploy paths RECORD, and they disagreed on three fields (`envVars`, `sourceHash`, instance `metadata.meshId`) — so a dashboard-added mesh could never be found stale and failed verification on every status request. Found in a user's logs. Fixed via a `captureMeshBaseline` dep at the cross-feature seam, with a parity test pinning the record.

### One status vocabulary, three tables — ✅ SHIPPED 2026-08-04 ([`../complete/2026-08-04-two-status-vocabularies-one-grid.md`](../complete/2026-08-04-two-status-vocabularies-one-grid.md))

Three tables collapsed to one: `core/ui/utils/statusVocabulary` holds label + severity per status, and three one-line adapters produce the `color` / `variant` / `dot` each surface wants. `MESH_MATRIX` and the grid's own table are gone; `meshStatusDisplay` is now a thin view over the shared entry. Two aliases collapse in one place instead of two (`normalizeDisplayStatus`), and `config-incomplete` stopped folding into `stale` — folding it would have relabelled an incomplete mesh "Update available". Visible change, accepted: `update-declined` and `config-incomplete` dots move orange → yellow, since severity is stored once and orange was not a severity but a second table's opinion. **Not** removed: the `config-changed` alias is the extension↔webview wire value (5 production sites), not merely a UI spelling — a separate change.

### A second, dead, unwrapped mesh-creation implementation — ✅ SHIPPED 2026-08-05 ([`../complete/2026-08-04-dead-second-mesh-create-implementation.md`](../complete/2026-08-04-dead-second-mesh-create-implementation.md))

Found by the org-targeting audit. `handleCreateApiMesh` is a second implementation of "create a mesh" — unwrapped by `withOrgContext`, and reachable from nothing (no webview sends `create-api-mesh`; the wizard executor calls the properly-wrapped `deployNewMesh`). It survives across six registration sites plus two docs that use it as a live example, which is enough to look alive under grep. Confirmed dead on four independent checks and removed. The cascade was larger than scoped: it was the sole member of `PROGRESS_CALLBACK_TYPES`, so `needsProgressCallback`, its config file and the streaming special case in `createProject.ts` went too — along with the `api-mesh-progress` message that branch sent, which had no listener. A dead sender and a dead receiver had been keeping each other looking alive.

### Eleven superseded message handlers, still registered — ✅ SHIPPED 2026-08-05 ([`../complete/2026-08-05-eleven-superseded-message-handlers.md`](../complete/2026-08-05-eleven-superseded-message-handlers.md))

Found by the first run of `dead-code-scan`'s closed-reference-loop procedure. Eleven message types sit in handler maps that nothing sends — no `ui/` sender, no MCP descriptor, no command, only plumbing and their own docblocks. Most are superseded rather than merely unreachable: `verify-github-repo` lost to a client-side check, `open-adobe-console` to `openDevConsole`, the two `cancel-*` messages to `cancel-project-creation`, the three DA.live ones to the `check-dalive-auth` generation. `get-dalive-sites` is also an outbound `sendMessage` nothing listens for — a dead pair keeping itself alive. All eleven removed, ~1500 lines across 23 files. `inspect-mcp` resolved as dead on the hand check with a twist — `ai/README.md` claimed the AI surface's Refresh action called it, and there is no Refresh action; the doc was asserting a caller that does not exist. `mcpInspector` survives (checked, not assumed): `clearMcpCache` and `inspectAllServers` still serve live callers.

### Storefront staleness misses the two PaaS endpoints — ✅ SHIPPED 2026-08-04 ([`../complete/2026-08-04-storefront-staleness-misses-paas-endpoints.md`](../complete/2026-08-04-storefront-staleness-misses-paas-endpoints.md))

**Verified by hand.** `STOREFRONT_CONFIG_ENV_VARS` omits `PAAS_GRAPHQL_ENDPOINT` and `PAAS_CATALOG_SERVICE_ENDPOINT`, both of which `configGenerator` reads on the PaaS path — so changing either changes what `config.json` would contain without marking the storefront stale. No prompt, no error, and the deployed storefront keeps serving the old backend. The ACCS arm is complete, which is why the common path hid it. Two hand-maintained lists over one schema. Both keys added; the durable half is `storefrontStalenessDetector-generatorCoupling.test.ts`, which drives the coupling mechanically — feed the generator nothing but what the detector watches, and every param the backend supports must still come out defined, so a param sourced from an unwatched key (including one that does not exist yet) fails. Audited both directions: all 14 watched keys are read, so there is no over-watching to trim.

### Cosmetic Adobe reads should be non-interactive — ✅ SHIPPED 2026-08-04 ([`../complete/2026-08-04-cosmetic-adobe-reads-should-not-prompt.md`](../complete/2026-08-04-cosmetic-adobe-reads-should-not-prompt.md))

Fallout from the same-day fix for "a browser opened mid-modal". Adobe entity reads come in two hazard levels — SDK-with-CLI-fallback (a stale token shells out to `aio console …` and opens a browser) and SDK-only (`getOrganizationsSdkOnly`, the P1 rule, returns `[]` instead) — but they are assigned by call site rather than by whether the user asked for the read. The sign-in guard now on `addIntegrationFlowHandlers` correctly converts the browser into an in-app prompt for user-initiated reads, which leaves background ones prompting too: `useWizardEffects` hydrates a project's display title in edit mode via `get-projects`, so a stale token interrupts wizard startup with a sign-in modal to fix a name-vs-title cosmetic. Strictly better than the browser it replaced, so a refinement rather than a regression. Shipped the same day it was filed. `getProjectsSdkOnly` / `getWorkspacesSdkOnly` join `getOrganizationsSdkOnly` — implemented as an `sdkOnly` flag that gates the existing fetch's CLI fallback rather than a duplicated read. A `quiet: true` payload flag opts a caller out of BOTH halves: `requireAdobeAuth` skips the prompt, and `handleGetProjects` routes to the SDK-only fetch. Opting out of the prompt is only safe because of that second half — either alone reintroduces a hazard (a prompt-free CLI-capable read opens a browser; an SDK-only read behind a prompt still interrupts). `useWizardEffects`' title hydration is the first caller.

### Console teardown pre-empts one delete blocker of three — ✅ CLOSED 2026-08-05 ([`../complete/2026-08-04-console-delete-blockers-not-preempted.md`](../complete/2026-08-04-console-delete-blockers-not-preempted.md))
...
### Face buttons vs the kebab — ✅ SHIPPED 2026-08-04 ([`../complete/2026-08-04-card-face-buttons-vs-kebab.md`](../complete/2026-08-04-card-face-buttons-vs-kebab.md))

Research item, then one cross-tile change. Raised by "why does a mesh get a Redeploy button — it should use the same kebab behavior"; two attempts to answer it on the mesh alone produced a mesh-only exception, which is the signal the question is about the shared verb vocabulary. Today the face carries Deploy/Update/Retry and the kebab carries Redeploy only on a healthy card, so in every state where something is wrong the same operation appears on the face under a different name. The asymmetry any decision must handle: integrations have four labels over TWO real calls (retry and redeploy differ), while every mesh verb resolves to one `onDeployMesh()`. Three options weighed. **Research complete 2026-08-04** (`.rptc/research/card-face-buttons-vs-kebab/research.md`): integrations is the ONLY surface in the extension with a face button — ProjectCard and PromptCard put every verb in the kebab and use the card body as the open-detail affordance — and its face button needs a `stopPropagation` wrapper to avoid firing the card's own click, which is the fingerprint of a control competing with its container. Adobe's Spectrum guidance agrees (cards stay simple; richer actions belong in the "more" menu or the detail side panel). Shipped as Option A in `495b8b78`: face buttons removed from both card kinds, every verb a kebab item through one `statusVerb()`. The never-deployed card puts its verb first in the kebab. Implementing it also exposed three defects in the previous day's mesh Remove — wrong component removed, card outliving the component, empty state losing its page chrome — all fixed in the same commit. Research doc retained at `.rptc/research/card-face-buttons-vs-kebab/research.md`.

### The add path bypasses the "one deploy-record writer" — ✅ DONE 2026-08-04 ([`../complete/2026-08-04-two-deploy-record-writers.md`](../complete/2026-08-04-two-deploy-record-writers.md))

`src/features/CLAUDE.md` documents `recordDeployOutcome` as "the one keyed deploy-record writer every deploy path lands on" — but `addAppBuilderComponent` builds its own state and writes it through `persistResult`, never touching it. That second writer is what dropped a failed add's error reason: `recordDeployOutcome` already merges `error` correctly (and clears a stale one on later success), while the add path reimplemented the write and omitted the field. Consolidated the same day: both paths record through `recordDeployOutcome`, and the three state builders plus `persistResult` are gone. The create case was harder than filed — `resolveKeyedComponentId`'s migration branch would have landed a second integration on the first one's key, so `create: true` bypasses it. One deliberate behaviour change: a redeploy now merges rather than replaces, so fields the deploy has no opinion about survive. Third instance of this shape on 2026-08-04 (two mesh resolvers, two status-dot implementations, this) — all three now single-source.

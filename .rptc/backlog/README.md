...
### Commerce-connect: connection/ownership model for multi-product Adobe demos ([`commerce-connect-aem-sc/`](commerce-connect-aem-sc/overview.md))

Build multi-product Adobe demos through **owner archetypes** (creation wizards): the **commerce-SC wizard** today, a **content-SC wizard** later — each *owns* its demo and *connects out* to systems usually **not built with the extension** (so **manual entry is the spine**; discovery/federated is the special case). **v1 anchors on the commerce-hub** — the first case *implemented* on a **product-neutral spine** (commerce is one product, not *the* product), not a privileged one; v1 also ships a **visible journey selector** (commerce live, content "coming soon") and **one live outbound spoke — commerce→AEM content** (AEP/App Builder stay designed slots). The **content-SC owner wizard** and the **federated** peer-to-peer case (Team-facing) become **first-class** later and must drop in additively. Reuses the existing project configuration model (`selectedStack` / `componentSelections` / `componentConfigs` / the registry) — **no new manifest field**, no multi-IMS refactor; neutrality stays structural, not a built-ahead framework (YAGNI). Designed (research/decision trail in git history); paused. Start the [`overview.md`](commerce-connect-aem-sc/overview.md) → the organizing model ([ownership-vs-connection](commerce-connect-aem-sc/ownership-vs-connection.md)), the per-archetype [user journeys](commerce-connect-aem-sc/user-journeys.md), the [roadmap](commerce-connect-aem-sc/roadmap.md) (P1 discover · P2 apply → H1 model · H2 dashboard), and the TDD-ready [discovery plan](commerce-connect-aem-sc/slice1-discovery.md). Architecture seam: [ADR-003](../../docs/architecture/adr/003-multisite-architecture-seam.md).

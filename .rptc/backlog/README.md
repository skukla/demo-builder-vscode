...
### The add path bypasses the "one deploy-record writer" ([`2026-08-04-two-deploy-record-writers.md`](2026-08-04-two-deploy-record-writers.md))

`src/features/CLAUDE.md` documents `recordDeployOutcome` as "the one keyed deploy-record writer every deploy path lands on" — but `addAppBuilderComponent` builds its own state and writes it through `persistResult`, never touching it. That second writer is what dropped a failed add's error reason: `recordDeployOutcome` already merges `error` correctly (and clears a stale one on later success), while the add path reimplemented the write and omitted the field. Not fixed with the bug because the canonical writer merges onto an EXISTING entry and a failed add has none — it needs a create case first, which is a refactor rather than a fix. Moderate risk, well covered by the two runner suites. Third instance of this exact shape on 2026-08-04 (two mesh resolvers, two status-dot implementations, this).

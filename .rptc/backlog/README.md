...
User-confirmed 2026-07-15: the product noun for the custom, action-carrying integration is **"custom integration"** (ERP / CRM / Firefly), never "App Builder app" (that's the impl substrate). The user-facing string rename **shipped this session** (KindStage/CustomStage/IntegrationsStep/AppBuilderCard/integrationRows/deployApp/appDeployment/appComponentManager + the blank-shell catalog name); internal identifiers kept. Two follow-ups captured: **(2) ✅ SHIPPED 2026-07-15 (`f91669cb`)** — `appState` was set + saved but `writeManifest` never serialized it (loader reads it), so a deployed custom integration lost its URL/status on reload; now persisted, mirroring `meshState`. The extras (`appBuilderComponentSources`/`additionalConsoleApis`) need a writer **and** loader change → folded into §E. **(3)** the "plural custom integrations as packages inside one app" model = the existing **slice 3 (package-bound)** work, reframed in the confirmed language (does not fork it). Cross-links §A slices + §E ([`2026-06-21-appbuilder-component-first-class-persistence.md`](2026-06-21-appbuilder-component-first-class-persistence.md)).

...
### Storefront staleness misses the two PaaS endpoints ([`2026-08-04-storefront-staleness-misses-paas-endpoints.md`](2026-08-04-storefront-staleness-misses-paas-endpoints.md))

**Verified by hand.** `STOREFRONT_CONFIG_ENV_VARS` omits `PAAS_GRAPHQL_ENDPOINT` and `PAAS_CATALOG_SERVICE_ENDPOINT`, both of which `configGenerator` reads on the PaaS path — so changing either changes what `config.json` would contain without marking the storefront stale. No prompt, no error, and the deployed storefront keeps serving the old backend. The ACCS arm is complete, which is why the common path hid it. Two hand-maintained lists over one schema; the durable fix is deriving one from the other, or a test that fails when the generator reads a key the detector does not watch.

...
### Aio-CLI prereq check is too shallow ([`2026-06-10-aio-cli-prereq-check-too-shallow.md`](2026-06-10-aio-cli-prereq-check-too-shallow.md))

Surfaced from a field issue on `v1.0.0-beta.113`. The aio-cli prereq runs only `aio --version`, which returns the bundled version string without touching config, network, or the Adobe API — so a binary whose config-load path is broken or whose plugin registry is corrupted gets marked installed when real ops (`aio config get`, `aio plugins:list`) hang indefinitely. Add a tier-2 functional check (`aio plugins:list --json` with a 5-second timeout) after the existing version check. Reports `installed-but-broken` distinctly from `not-installed` so users see an actionable reinstall message instead of mysterious downstream timeouts. Scope limited to aio-cli; other prereqs don't share the self-reflection-without-real-ops failure mode.

### Token validation timeout (TIMEOUTS.QUICK = 5s) too tight for slow networks ([`2026-06-10-token-validation-timeout-too-tight.md`](2026-06-10-token-validation-timeout-too-tight.md))

Same field issue as the aio-cli prereq depth bug. The `aio config get ims.contexts.cli.access_token --json` token-read at `tokenManager.ts:81-85` uses `TIMEOUTS.QUICK` (5s), which routinely fails for users on slow networks — the colleague's debug log shows the 3-retry loop bailing out and forcing re-login multiple times in a single session, with successful re-logins taking 25-60 seconds each. Mirrors the v1.5.0 `CONFIG_WRITE` precedent (also bumped 5s → 10s for the same reason). Add a dedicated `TIMEOUTS.TOKEN_VALIDATION = 10000` constant in `timeoutConfig.ts`, switch the call site, update the one assertion in `tokenManager.test.ts`. Don't touch the retry-loop shape — it's right; only the per-attempt timeout is too tight. ~10 minutes.

### Prereq install timeout — surface "check power state" hint ([`2026-06-10-prereq-install-low-power-hint.md`](2026-06-10-prereq-install-low-power-hint.md))

Field follow-up — same colleague. Her aio CLI install took 4m 43s (vs the 60s estimate) and the api-mesh plugin install timed out at 180s, all because her MacBook was critically low on battery and macOS was throttling CPU + network + disk by ~4-5x. The user-facing error was the raw `Command timed out after 180000ms` — accurate but unactionable. Add a one-line hint to the install timeout error formatter: "If your laptop battery is low, plug in and retry — macOS throttles CPU and network under low-power conditions." Always-fired (no battery-state detection), low signal-to-noise since "plug in" is never bad advice during a multi-minute install. ~15 minutes.

### Aio-CLI install isn't idempotent — partial plugin failures aren't retryable ([`2026-06-10-aio-cli-plugin-install-not-retryable.md`](2026-06-10-aio-cli-plugin-install-not-retryable.md))

Same field case. The aio CLI install completed cleanly (4m 43s), then the api-mesh plugin install failed mid-step. The wizard reported "Adobe I/O CLI installation did not complete" — but the CLI binary IS installed; only the plugin is missing. The Install action treats CLI + plugin as one atomic block via `installHandler.ts:541 → installPlugins`, so retrying re-runs everything (including the now-redundant 4m CLI install) or doesn't run at all. Make the install handler smart: before running the CLI install command, check if `aio --version` succeeds; if yes, skip straight to plugin install. UI surface stays the same. Composes with the low-power hint + tier-2 check entries — ship in any order. ~30-60 minutes once the model is settled.

...
Deferred during BYOM PDP routing Phase 1. When an SC deletes a SKU, the cached PDP URL still serves the template and the drop-in queries Commerce, gets nothing back, renders empty. Cleanup tooling (Refresh PDPs action, action-side telemetry) was rejected as overkill for the actual demo-workflow frequency. The drop-in showing a clean "Product not available" instead of broken empty content handles the same degradation with less infrastructure and covers more cases (any Commerce-returns-nothing scenario, not just deletion). ~2 hours, lives in storefront-template code OR as a Demo Builder code patch (depends on thin-layer evaluation outcome). Cross-references the drop-in version coupling finding — citisignal's drift may affect the empty-state detection API.

...
### One status vocabulary, three tables ([`2026-08-04-two-status-vocabularies-one-grid.md`](2026-08-04-two-status-vocabularies-one-grid.md))

**The wording half shipped 2026-08-04** — mesh labels are now bare state names matching the integration vocabulary word for word, and the projects-list card composes `Mesh · <state>` itself. What remains is structural: one status fact encoded in three tables (`MESH_STATUS_DISPLAY` → text/color/variant, `INTEGRATION_STATUS_DISPLAY` → label/dot, `MESH_MATRIX` → dot), with the mesh card taking its LABEL from the first and its DOT from the third — agreeing only by coincidence. The three prop shapes are a thin excuse: `variant` and `dot` are near-identical and `color` is a parallel encoding of the same severity. Collapse to one table keyed by `CardStatus` plus three one-line severity→prop adapters, with a contract test; keep the copy byte-identical.

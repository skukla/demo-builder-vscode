...
### Mesh deployment state — one fact, five readers, two writers ([`2026-08-04-mesh-deployment-state-one-accessor.md`](2026-08-04-mesh-deployment-state-one-accessor.md))

Six commits on 2026-08-04 fixed six mesh bugs that were all the same bug: two writers of "is this mesh deployed, and which mesh is it", and a different reader consulting the wrong one each time. Each fix moved the failure one reader downstream — the add picker read a hand-authored catalog, `aio api-mesh` read a `.env` nobody wrote, `getMeshComponentInstance` read an absent instance, `handleRequestStatus` read an instance status stuck at `ready`, `hasMeshDeploymentRecord` read a staleness baseline, `getMeshStatusKey` re-derived the resolver's fallback half. Every one was found by a user reporting a symptom; none by review or tooling, because each site reads as correct in isolation. Fix is the `progressRegister` treatment: separate the four questions being conflated (which entry / has deployed / is deployed now / is stale), one accessor each, delete `deployMeshHeadless`'s now-redundant direct status writes, and a contract test that fails on every surface when the shared answer changes.

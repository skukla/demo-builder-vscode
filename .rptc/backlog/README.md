...
Build multi-product Adobe demos where each product is **owned** (extension provisions it) or **connected** (referenced via coordinates). **Connection is the primitive** — populated by **manual entry** (general) or **discovery** (convenience) — so no partner SC is forced to use the extension. **v1 anchors on the commerce-hub** (an owned commerce demo connecting out to partner-managed Adobe apps); the **AEM-SC federated** case (owned `aem-boilerplate-xcom` storefront ← *discovered* commerce backend, authored in Universal Editor) is a later milestone on the same primitive. Reuses the existing project configuration model (`selectedStack` / `componentSelections` / `componentConfigs` / the registry) — **no new manifest field**, no multi-IMS refactor. Designed (research/decision trail in git history); paused. Start the [`overview.md`](commerce-connect-aem-sc/overview.md) → the organizing model ([ownership-vs-connection](commerce-connect-aem-sc/ownership-vs-connection.md)), the [roadmap](commerce-connect-aem-sc/roadmap.md) (P1 discover · P2 apply → H1 model · H2 dashboard), and the TDD-ready [discovery plan](commerce-connect-aem-sc/slice1-discovery.md). Architecture seam: [ADR-003](../../docs/architecture/adr/003-multisite-architecture-seam.md).

...
Let **two solutions consultants** — a **Commerce SC** (owns the commerce backend) and a **Content SC** (owns the content, authored in **AEM Sites**) — build **one demo storefront together** despite working in **separate Adobe accounts**. For each pairing there's **one master storefront**; each SC has their **own copy** that **auto-syncs** from it. The Commerce SC adds the **commerce parts from the commerce boilerplate** into the master (so they flow to both copies); each copy is wired to the Commerce SC's commerce backend (which reads fine across accounts). Separate-copies-auto-synced is the only workable shape *because* the two SCs are in different accounts. **Re-aimed 2026-06-03** — earlier drafts framed this as a single commerce demo "connecting out" to AEM content; that was the wrong target and is superseded. Reuses existing machinery (the sync engine, the block/feature-pack installers, Connect-Commerce). Start the [`overview.md`](commerce-connect-aem-sc/overview.md) → the [roadmap](commerce-connect-aem-sc/roadmap.md) (master+copy → add commerce parts → wire backend → AEM content → cross-team access). The other docs in the folder carry older framing — see the overview's table. Architecture seam: [ADR-003](../../docs/architecture/adr/003-multisite-architecture-seam.md).

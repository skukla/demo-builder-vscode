...
### MCP inspector — spawn diagnostics & third-party env ([`2026-05-28-mcp-inspector-spawn-diagnostics.md`](2026-05-28-mcp-inspector-spawn-diagnostics.md))

Two adjacent fixes in `src/features/ai/mcpInspector.ts`. (1) The transport pipes stderr but nothing drains it, so spawn failures surface only the JSON-RPC code (e.g. "Connection closed") and the user has no actionable detail — drain the buffer and append a 4 KB tail to `McpInventoryEntry.error` on both error and timeout outcomes. (2) The SDK's env allowlist is correctly restrictive for Adobe token safety but strips well-known *configuration* vars some third-party MCPs (notably Playwright) need at startup — extend with a short credential-free list (`NODE_OPTIONS`, `PLAYWRIGHT_BROWSERS_PATH`, `XDG_CACHE_HOME`, etc.). Self-contained, single file, ~60 lines plus tests.

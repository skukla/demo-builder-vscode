...
**Measured, and the count is the point: five of seven producers declare a payload type, and not one reaches the component that reads it.** `getInitialData` is abstract as `Promise<unknown>` (`baseWebviewCommand.ts:166`), each named type — `ConfigureInitialData`, `AiOverviewInitialData`, `InitialWizardData` — appears in exactly ONE file, its producer, and the cast at `baseWebviewCommand.ts:213` widens whatever was declared to `MessagePayload` one line after it is produced. So producer and consumer agree by convention, and a declared type is worse than `Record<string, unknown>` here because it LOOKS checked. Same shape as the `stackBackend` casts CLAUDE.md records, milder blast radius. This is the gap the project-title drift came through: `getProjectDisplayName` existed and was still missed four times, and the brand added in `95bdf0b0` cannot cross this boundary. Fix is mostly mechanical — move the interfaces somewhere both bundles import, write the two missing shapes, generify `BaseWebviewCommand` over its payload — but no bug is attached to it now, which is why it is here. Filed 2026-08-20.

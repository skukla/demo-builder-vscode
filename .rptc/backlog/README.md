...
#### Proxy: fall through to discovery when the pinned socket is gone ([`2026-08-08-mcp-proxy-pin-fallthrough.md`](2026-08-08-mcp-proxy-pin-fallthrough.md)) — **fix immediately**

`resolveProxyTarget` uses the `DEMO_BUILDER_MCP_SOCKET` pin verbatim with no existence check (`mcpSocketDiscovery.ts:136-138`), and every project's `.mcp.json` carries one. A pin whose socket is gone therefore never reaches the liveness-probing discovery branch below it — the proxy burns its full ~23s ENOENT retry window and exits, while a live server sits one branch further down. Sibling of the 2026-08-08 socket-unlink fix: that one stops a dead socket being created, this one stops a dead socket being fatal. Same fallthrough is needed in `mcpInspector`'s direct-probe branch. **No `AI_CONTEXT_VERSION` bump and no regeneration** — the socket path scheme is deliberately unchanged, which is what lets existing configs heal themselves.

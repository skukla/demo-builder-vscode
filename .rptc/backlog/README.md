...
### A second, dead, unwrapped mesh-creation implementation ([`2026-08-04-dead-second-mesh-create-implementation.md`](2026-08-04-dead-second-mesh-create-implementation.md))

Found by the org-targeting audit. `handleCreateApiMesh` is a second implementation of "create a mesh" — unwrapped by `withOrgContext`, and reachable from nothing (no webview sends `create-api-mesh`; the wizard executor calls the properly-wrapped `deployNewMesh`). It survives across six registration sites plus two docs that use it as a live example, which is enough to look alive under grep. Delete outright per no-soft-deprecation — but re-verify deadness first, because if it IS reachable the task inverts: an unwrapped `aio` create is precisely the 2026-08-03 incident where a deploy landed in a DELETED project for two days.

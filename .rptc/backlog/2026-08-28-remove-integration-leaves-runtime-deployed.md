...
- 2026-08-28  Live-proof blocker narrowed by elimination (2026-08-28): getServicesForOrg fails ONLY from inside the extension host. Discriminator matrix — raw curl outside: 200 (48s cold); the SAME @adobe/aio-lib-console SDK with the SAME key outside: 200 (98 services, 52s); the extension (which reads the SAME CLI token per tokenManager.ts:102): 500 twelve of twelve across two hours, THROUGH the new in-fetch retry, surviving host reloads. Other Console endpoints work fine from the host (projects/workspaces sub-second). Every code-side suspect eliminated: token source identical, api key identical, SDK identical, our timeout (60s) above the measured 52s. Remaining suspects are host-environment-specific: VS Code's extension-host network layer (proxy support patches fetch/http) or a request-shaping difference only manifest under their degraded backend. Next probe when picked up: log the exact request the SDK sends from the host (headers minus auth values) and diff against the working outside call; or simply retry after Adobe's backend recovers — if it works then, this was their degradation being sensitive to some host-side request property. AB-7's FIX is unaffected (unit-proven); only the live proof waits.
- 2026-08-31  2026-08-31  STALENESS CHECK (loop pickup): the item read 'backlog' while its fix had been on develop since 2b5be4ce0 on 2026-08-28, with appBuilderComponentRunner-runtimeVerify.test.ts covering it. Status corrected to built. Nothing about the CODE is outstanding; what remains is live proof, which needs a working Adobe Console and a real project, so it is the owner's to run and cannot be done unattended. Re-triaged: lane 2, parked at the supervised edge.
- 2026-08-31  docs(backlog): three items were understating themselves, one of them the bug I was about to fix (`1a1c4bd7e`)
- 2026-09-10  2026-09-10  HALF THE BUG WAS STILL LIVE. The runner's fix was real and unit-proven, but handleRemoveAppBuilderComponent discarded result.runtimeCleanup and returned a bare {success:true} — so verified:false and failed[] ('STILL RUNNING' per the runner's own comment) both reached the user as clean success. GuardableResult had no field for it, which is why nobody saw it. Fixed: warning toast for the SC, data.runtimeCleanup for the agent. 3 regression tests, all proven to fail against the old handler.
- 2026-09-10  fix(app-builder): AB-7's other half — the handler threw the evidence away (`aa18868ad`)

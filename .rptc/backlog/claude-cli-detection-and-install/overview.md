...
| **"Binary missing → notification → action" pattern** | The `storefront-setup-github-app-required` flow in `storefrontSetupPhase3.ts:140-158` (AEM Code Sync install prompt) | `src/features/eds/handlers/storefrontSetup/storefrontSetupPhase3.ts` | This is the template. We mirror the same notification shape ("isTeamOrg" style) and action handling for `openInClaude.ts`. Don't invent a new notification UX. |

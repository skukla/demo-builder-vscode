...
6. **Set up the Commerce Prerenderer** for product detail pages; confirm PDPs render.
7. **CORS:** allow-list the storefront domain on the backend.
8. **Verify transact:** open the published site → product list (Live Search/Catalog) → PDP → add to cart → reach checkout.
...
- **New wrinkle:** PDPs now come from the **AEM Commerce Prerenderer** (folder mapping deprecated) — confirm in the spike.

...
- 2026-08-27  Follow-up: narrowing the gate without a trigger to re-evaluate it was half a fix. Adding an App Builder integration changes composition, and nothing re-derived the skill set — the sweep only fires on a version bump, the freshness badge only on a missing package (none, since commerce-extensibility was already installed for the storefront). So the integration arrived with no App Builder skills. Removal was broken longer: the last component leaving left its skills forever. Both paths now refresh tiers 1+2 through an injected dep, best-effort, after the save. Four tests, two of which fail without it.

...
| Destination move | `destinationHandlers.ts`, `appBuilderComponentMigration.ts`, `set_project_destination` | **Decided 2026-09-21.** Within the same Adobe project, an integration in its own workspace stays put and its APIs stay off the project's workspace (built). Into ANOTHER Adobe project (owner, 2026-09-21): each group is removed from the old Adobe project — Commerce clean-up, uninstall, old workspace deleted — and added in the new one, with its folder on disk kept (built, `componentRelocation.ts`). The new workspace is made FIRST, so a clean-up that cannot finish is undone with nothing changed. The handler confirms this one kind of move, since it deletes live resources. Not yet run live. |
...
- 2026-09-21  fix(app-builder): a removal says what each phase is doing; a slow workspace delete is waited for (`5f05a7007`)

...
- 2026-08-27  2026-08-27: the flagged check/continue inconsistency INVESTIGATED and confirmed a real bug — continue demanded per-node variants on EVERY Node major while check narrows by requiredFor. Live path: headless stack (frontend Node 24, mesh Node 20) + aio-cli (plugins requiredFor the mesh) → green check, then Continue flips to a blocking 'error' over Node 24, which nothing required. Fixed: resolveRequiredMajors moved to shared.ts, continue uses it (TDD: failing scope test first, continueHandler-perNodeScope.test.ts). Four checkHandler delegation-spy assertions converted to effect assertions — the seam moved module-internal, the mock-audit rule's textbook case.

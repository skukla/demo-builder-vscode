...
|---|---|---|---|
| D1 | **Decided 2026-09-25: no new settings**; each row became a fixed rule shown on the Mapping tab (see AB-26w log). Was: Which business-user settings to add to the ERP integration | Ship rows 1, 2, 3 and 4 of the design (note visibility, customer emails, what a credit rejection does, hold in Commerce); defer 5 (invoice capture) and 6 (write-back switches) until a demo asks | AB-26w |
| D2 | **Decided 2026-09-25**: no experiment (owner saw events fail without the id and flow with it). Build: set provider_id through PUT V1/eventing/updateConfiguration (documented field, all optional) in the integration's own install step after Adobe's installer; Demo Builder if App Management allows no extra step (unverified). Prove on Bodea by blanking the field and letting the install set it. Was: Provider ID: run the reversible experiment on Bodea (blank the Admin field, save, change a product, watch the registration tracing, paste it back) | Yes, with you watching; if events still flow, the field is cron-only and the extension's check becomes "advisory"; if they stop, the extension writes the id after install | AB-26x |
| D3 | **Yes (status); capture timing decided 2026-09-25**: not a setting but a mapping shown on the Order card: an ERP invoice creates the Commerce invoice with capture; capture at ERP confirmation and at ERP shipment dropped (no precedent, `erp-order-to-cash-capture`). Earlier note: create "Confirmed in ERP", available under Pending and, if Commerce allows, Processing (unverified). When Commerce invoices and captures (at ERP invoice / at ERP confirmation / at checkout) waits on order-to-cash research, then on whether a business user should choose it in the integration's settings. Was: The "Confirmed in ERP" custom order status on Bodea | Create it (two minutes in Admin: Stores → Order Status, code `erp_confirmed`, assign to Pending, not default) and set "Order status when the ERP confirms" — it is what makes the confirmation visible in the Orders grid | AB-26x, setup guide |
| D4 | **Yes, 2026-09-25**: a setup checklist in Demo Builder on the ERP pair's tile (steps opened, checked, marked done, dismissed); the integration keeps none, its README says what to do after install (AB-26x log). Was: Build the manual-step surfaces: a check on the integration tile, a Setup card on the Admin page's Mapping tab, the list in the AI bundle skill | Yes, as the next extension slice after the matrix closes | AB-26x |
...
| D6 | **Yes, 2026-09-25**: watch during D9's fresh add; if still required, switch the ERP off for a minute and add to a cart to see whether it matters, then take the evidence to Adobe. Was: The webhooks' `required: true` on the instance (source says `false`) | Investigate on the next remove + add (a fresh install writes the source value); if it persists, ask Adobe | ledger |
| D7 | **Yes, 2026-09-25**: when Commerce does not answer, find the order in the ERP by its Commerce number and show what is known, saying Commerce's part is missing. Was: `get_erp_order_trace` answers nothing when the Commerce read times out | Add the ERP fallback (find the order by its Commerce number) — small, in both repos | AB-26e |
| D8 | **Yes, 2026-09-25**: record the order as sending before the ERP call and update it after each step (failure shows where, with Retry); add a "Sent to ERP, waiting for confirmation" comment on the Commerce order. Visibility only: no status changes in either system. Was: The integration's history records nothing for a run that fails after the ERP call (the Admin page showed no "sent") | Record the outcome before the write-back, update it after — small | AB-26e |
| D9 | **Split, 2026-09-25**: remove + fresh add now, in the supervised Bodea session (tests the install path and lets D6 and D2 be observed); the reset proof after AB-26y steps 1 to 3 land, so it proves the reset being kept. Was: Run the destructive half of the matrix now: reset, then remove_integration, then a fresh add (wipes ERP data, ledger, test orders' ERP numbers) | Yes, now, before the demo data is curated; it is the reversibility proof the demo depends on | AB-26e |
| D10 | **Yes, 2026-09-25**: keep group 18 and the contract price (demo setup); prices and stock come back with the later reset proof; in the Bodea session, cancel in Commerce any test order still open (releases reserved stock, clears the ERP work list), leave invoiced ones. Was: Test data left on Bodea: customer group 18, contract price 40 on accessmesh, list price 55, stock 77 and 61, four test orders (two cancelled) | Keep the group and the contract price (they are the demo setup); let the reset restore prices and stock; leave the orders (Commerce cannot delete them) | AB-29 |
...
| D14 | **Dropped 2026-09-25**: the refresh job becomes part of the reset copy (AB-26y); no polling between the systems. Was: The ERP's minute refresh job (companies, credit, stock from Commerce) runs 30–120 s on this sandbox and 13 of the last 50 runs died at its 120 s limit; a new run starts every minute whether or not the last one finished, so two or three overlap and hammer the same slow Commerce — a likely share of today's 503s | Fix now in the integration: skip a run while the previous one is still going (a lock in State with a TTL), and run every two minutes; both small | AB-26e |
| D15 | **Yes, 2026-09-25**: on a product event, read the product's current state from the ERP and apply that, not the event's values. Was: ERP → Commerce product updates can apply out of order: two quick ERP edits (rename, then restore) each raised an event; both handler runs hit the 60 s limit on the slow sandbox and were redelivered, and the older event's retry landed last, so Commerce kept "Access Point Pro" | Fix in the integration, the way the hold handler already works (rule M2): before writing, read the product from the ERP and apply the ERP's CURRENT name, price and stock, not the event's values; then no redelivery order can leave Commerce behind the ERP. Small | AB-26e |

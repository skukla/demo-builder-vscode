...
| D4 | **Yes, 2026-09-25**: a Setup section on the integration's Admin page whose steps can be done, verified or marked done, and dismissed; demo-only steps on the Demo Builder tile (AB-26x log). Was: Build the manual-step surfaces: a check on the integration tile, a Setup card on the Admin page's Mapping tab, the list in the AI bundle skill | Yes, as the next extension slice after the matrix closes | AB-26x |

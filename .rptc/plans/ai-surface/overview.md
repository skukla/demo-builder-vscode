...
| 3 | 🟡 **Enforcement** — ceiling table ✅, catalog pin ✅, **envelope test still missing** | What stops the surface degrading again. The ceiling table asserts its own coverage in both directions, and `actionDescriptors.test.ts` pins the component catalog. What remains is one guard: nothing asserts that every tool answers in the `{content:[{type:'text'}]}` envelope. Checked 2026-08-17 — no such suite exists (42 test files searched, with a control). | not written — after phase 2 |

...
| ✅ `find_storefront_name_mismatches` + `migrate_storefront_name` | **Built as a PAIR, not the single bulk tool named here.** The bulk shape cannot carry the name echo an irreversible action requires — there is no single name to echo — and it would give an agent one call that deletes N DA.live site roots. So the sweep is a read and the migration takes one project, `confirm:true` + `confirmName` = the PROJECT name. Preceded by a second extraction (`storefrontNameMigrationForProject`), needed for a sharper reason than the repair's: the migration mutates the manifest in memory and returns WITHOUT saving, and the re-register DESTROYS the site publish key, so the real operation is migrate → persist → re-mint. Both extra steps were private methods of a command class, and a second implementation omitting them would look correct while leaving every migrated storefront unable to publish. The command's 16 existing tests pass unmoved. |

...
> - **R1 (AEM auth model):** ✅ **Verified (2026-06-11; full research: `.rptc/research/aem-content-source-auth/research.md`).** Key finding: the native AEM-Sites flow is **push-based** — on publish, AEM pushes content into the EDS pipeline, which reads back markup from `…/bin/franklin.delivery/<owner>/<repo>/main` on the AEM **author** host. That read is authorized by a **Site Authentication Token tied to an OAuth Server-to-Server technical account, configured once *inside AEM*** (Tools → Cloud Services → EDS Config → Authentication). Consequences:
>   - **(A) reuse `aio`/IMS identity is DEAD** — the user token is the Universal Editor *write-back* leg, not the EDS read. Verdict from research: No.
>   - **JWT Service Account is fully EOL** (past 2025-06-30) → any S2S is **OAuth S2S**.
>   - **The realistic ideal: the extension passes NO content-read token.** AEM owns it server-side. `getContentSourceAuthorization()` → **`null`** for AEM; the extension instead **guides + verifies** the one-time in-AEM EDS auth config. The `ContentSource` port already absorbs this (null today; can return an S2S Bearer if the BYOM path turns out to be needed — live-test item #3).
>   - **Residual uncertainty (Medium confidence):** whether the native push path ever uses `x-content-source-authorization` (BYOM) vs. purely the stored Site Auth Token. 5 live-test items listed in the research doc, runnable against the available AEM instance.
>   - ⏳ **PENDING PM framing decision:** adopt the native "AEM-owns-auth + extension guides/verifies" working model (port returns null; add a guide/verify responsibility), validating the exact mechanism via a live probe — see the new decision block below.

...
> - **R1 (AEM auth model):** ⏳ **Ideal-first; verification running (2026-06-11).** PM decision: **no throwaway dev-token quick-start** — settle and build the ideal experience first. The choice is a clean either/or, both behind the credential-agnostic `getContentSourceAuthorization()` port and never the DA.live bookmarklet pattern:
>   - **(A) Reuse the wizard's existing `aio`/IMS identity** — the genuine ideal: zero new credential steps. Build this **if** the verification confirms a user IMS token carries the AEM content-read scope.
>   - **(B) OAuth Server-to-Server**, set up once via a guided card + stored in VS Code `secrets` — the ideal **if (A) is infeasible** (a real credential; modern S2S, NOT the deprecated JWT Service Account).
>   The running research pass decides A vs B by answering *what token the AEM-Sites content source actually accepts*. Step 04/Step 07 finalize once it returns; **no F5 until the chosen ideal is built** (the writer's manual-author fallback — decision #2 — is separate and unaffected).

...
- ✅ **Screen chrome (2026-06-07).** Re-skinned `JoinStorefrontScreen` to the shared single-purpose-webview shell — `PageHeader` (title) + `content-area` + `SingleColumnLayout` + `PageFooter` (Continue/Join in the footer) — matching `ConfigureScreen`. **No timeline:** the launcher is one step; the timeline lives in the seeded Create wizard it opens. *(F5 caught the original `<View>`-only screen lacking the shared chrome — the screen's DoD only required prop-driven behavior + unit tests, which don't assert layout. DoD now includes the shared shell.)* +1 header-chrome test; 6/6 join-screen + gates green.
...
- ✅ **Home-screen "Join" entry (2026-06-07).** Wired the discoverable entry (was Command-Palette-only): a **"Join a Shared Storefront..."** item in the projects-home **"New" dropdown** *and* a sibling button on the **empty state** (first-run content SC has zero projects) → `onJoinStorefront` prop → `joinStorefront` message → `handleJoinStorefront` → `demoBuilder.joinStorefront` command. Reuses the existing `buildMenuItems` / `onAction` / handler-map pattern (mirrors Copy/Import). 4 new tests (handler routes the command; menu item present/absent; empty-state button fires) + handler-count bump (22→23) + 267 projects-dashboard/join regression + tsc/eslint/compile green.
- **Remaining (follow-ups):** `onConfirm` → **gallery-less seeded wizard launch** ✅ (shipped under Step 4c.3) and the **finalization hook** calling the marker writer ✅ (shipped as executor Phase 5c, Step 4). *(All Step 2 follow-ups now landed; live behavior is F5.)*

...
- **Remaining:** (b) persist `flow`/`upstream` on the saved content project; (c) thread `upstream` from the join descriptor into the wizard→`edsConfig` payload (F5 path) — which also sets the satellite's `repoUrl` to the upstream (clone source) while the new guards keep Phase 5/5b safely off.

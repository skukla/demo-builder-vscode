...
- **Remaining:** (a) executor Phase 5 (`config.json`→GitHub) gated off for satellites — *pending confirmation the satellite flow reaches `executor.ts` at all*; (b) persist `flow`/`upstream` on the saved content project; (c) thread `upstream` from the join descriptor into the wizard→`edsConfig` payload (F5 path).

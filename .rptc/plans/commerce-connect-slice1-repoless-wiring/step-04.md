...
- **Remaining (c):** (2) edsConfig assembly (descriptor → `edsConfig.upstream`/`repoUrl`/coords); (3) `join-confirm` → `createProject({joinDescriptor})` handoff + `execute` option; (4) WelcomeStep gallery suppression (flow+upstream). Items 3–4 are partly **F5-gated** (cross-webview launch + render).

...
- ✅ **registerSite no-false-success hardening (2026-06-07).** F5 surfaced a swallowed failure: a DA.live token that expires mid-pipeline makes `getImsToken` throw *before* the HTTP call, so `makeRequest` returned `{success:false, statusCode: undefined}` — which fell through `performSiteConfigRegistration`'s 401-only `DaLiveAuthError` conversion into a **warning**, so `withDaLiveAuthRetry` never re-authed and `executeSatelliteSetup` logged "Repoless satellite registered" as a **false success** (registration is the satellite's whole deliverable). Fix: `ConfigurationService.makeRequest` now tags a missing/expired token as `statusCode: 401` (semantically correct — no token = unauthenticated), so the existing `401 → DaLiveAuthError → withDaLiveAuthRetry` machinery re-authenticates and retries, and **fails loud** (throws, aborting setup) if it can't recover. Fixes both the satellite and commerce registration paths (shared code). 20/20 config-service + 8/8 repoless (incl. 2 new: re-auth-retry recovers; unrecoverable fails-loud-no-false-success) + 859/859 EDS regression + tsc/eslint green.

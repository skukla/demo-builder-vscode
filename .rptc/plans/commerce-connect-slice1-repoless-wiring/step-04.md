...
- **🎉 (c) complete — the join flow is wired end-to-end and builds clean.** Remaining for a real end-to-end run: the **marker write-side** (so a Commerce storefront carries `storefront-share.json` and is resolvable — pending the trigger decision) + Step 7 dashboard routing. Both F5.

...
| E · Commerce Admin | history, retry, one-order trace; **the entity map** — the Mapping tab replaces Settings: one card per composite entity, both systems side by side, ownership arrows, the join's setting on the card, sync per direction, ERP figures (AB-26m step 1, 2026-09-24) | the company / SKU lookup on the cards (AB-26m step 2) | ◆ a person looks at the live Admin page |

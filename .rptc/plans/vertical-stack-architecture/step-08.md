# Step 8: Tool Integration (commerce-demo-ingestion)
...
- **commerce-demo-ingestion:** `https://github.com/skukla/commerce-demo-ingestion`
  - Purpose: Data ingestion and cleanup scripts for Commerce and ACO
  - Branch: `main`
  - **Commands:**
    - `npm run import:commerce` - Import to Adobe Commerce (stores → customer groups → product attributes → categories → products → images → customer attributes → demo customers)
    - `npm run delete:commerce` - Reverse-dependency deletion of project-specific Commerce data
    - `npm run import:aco` - Import to ACO (metadata → products → variants → price books → prices)
    - `npm run delete:aco` - ACO data cleanup
    - `DRY_RUN=true npm run delete:*` - Test deletion without removing data

- **vertical-data-citisignal:** `https://github.com/skukla/vertical-data-citisignal` (or similar)
  - Purpose: CitiSignal demo data (products, categories, prices)
  - Branch: `accs` (ACO-specific data format) or `main` (Commerce format)

**Smart Detection:** Uses attribute prefixes (e.g., `br_*`, `cs_*`) to identify project-specific data for safe deletion.
...
- [ ] **Test:** `should execute npm run import:commerce command`
  - **Given:** Tool installed and configured with Commerce credentials
  - **When:** `executeCommerceIngestion()` is called
  - **Then:** Runs npm run import:commerce in tool directory
  - **File:** `tests/unit/features/eds/services/toolIntegration.test.ts`

- [ ] **Test:** `should support delete:commerce command for cleanup`
  - **Given:** Tool installed and configured
  - **When:** `executeCommerceCleanup()` is called
  - **Then:** Runs npm run delete:commerce in tool directory
  - **File:** `tests/unit/features/eds/services/toolIntegration.test.ts`

- [ ] **Test:** `should support dry run mode for cleanup operations`
  - **Given:** Tool installed and configured
  - **When:** `executeCommerceCleanup({ dryRun: true })` is called
  - **Then:** Sets DRY_RUN=true environment variable before execution
  - **File:** `tests/unit/features/eds/services/toolIntegration.test.ts`
...
describe('ToolManager', () => {
    ...
    describe('Execution', () => {
        ...
        it('should execute npm run import:commerce command', async () => {
            ...
            // Act
            await toolManager.executeCommerceIngestion();
        ...
        it('should support dry run mode for cleanup operations', async () => {
            ...
            // Act
            await toolManager.executeCommerceCleanup({ dryRun: true });
...
export class ToolManager {
    ...
    async executeAcoCleanup(
        ...
        return this.executeToolScript('delete:aco', 'commerce-demo-ingestion', options);
    }

    /**
     * Execute Commerce data ingestion (convenience method)
     */
    async executeCommerceIngestion(
        options: ToolExecutionOptions = {},
    ): Promise<ToolExecutionResult> {
        return this.executeToolScript('import:commerce', 'commerce-demo-ingestion', options);
    }

    /**
     * Execute Commerce cleanup (convenience method)
     */
    async executeCommerceCleanup(
        options: ToolExecutionOptions & { dryRun?: boolean } = {},
    ): Promise<ToolExecutionResult> {
        if (options.dryRun) {
            // Execute with DRY_RUN environment variable
            return this.executeToolScriptWithEnv(
                'delete:commerce',
        ...
        return this.executeToolScript('delete:commerce', 'commerce-demo-ingestion', options);
    }
...
- [ ] vertical-data-citisignal cloned with appropriate branch (accs or main)
- [ ] Tool updates on use (git pull) with 24-hour TTL
- [ ] .env file generated with ACO and/or Commerce credentials
- [ ] ACO operations: `npm run import:aco` and `npm run delete:aco`
- [ ] Commerce operations: `npm run import:commerce` and `npm run delete:commerce`
- [ ] Dry run mode for safe deletion testing (`DRY_RUN=true`)

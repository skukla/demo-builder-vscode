...
- **Remaining (final Step 2 increment — integration glue, typecheck-gated):** the Join webview command/entry registration (`BaseWebviewCommand` + `package.json` contribute + webview/webpack entry + home-screen "Join" entry); `onResolve` → `createPublicMasterReader((url)=>fetch(url))` + `handleResolveJoinLink`; `onConfirm` → gallery-less seeded wizard launch (flow/upstream/coords); and the **finalization hook** that calls `writeMasterMarker` (via a `GitHubFileOperations.createOrUpdateFile` adapter) for shareable storefronts. The testable seams are all done + green; what's left is wiring + registration.

...
8. **integrationRows.ts + IntegrationResultRow (+ MeshApiEnableRow move)** — resolver incl. mesh via the BOTH-key check (package-seeded mesh yields a row with `needsSetup`), kind labels, the APIs in use; row anatomy incl. the "Not set · Set up" state; Remove routing per kind.
...
4. Add a catalog integration (later add): kind → catalog pick → "Deploys to" summary → API stage (locked entries checked+disabled, pick a free API) → Add; row appears with the APIs in use listed by name.

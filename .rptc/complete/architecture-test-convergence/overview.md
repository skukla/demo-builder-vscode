...
| 9 | **Does 93% hold outside the modules we trusted? (PL-22)** | **ANSWERED 2026-08-30 — NO** | 59.29% over a 7-module sample vs the pilot's 93.37%. Sample chosen by a deterministic stride across all 423 source files that have a test — NOT "modules nobody is confident about" as scoped here, because that is a biased selection too and would have answered a different question. `envMerge` re-run as a control reproduced 100% exactly, so the gap is real. Score falls monotonically with `await` count: `installHandler.ts` (41 awaits) 41.77%. The finding is that tests fail to constrain ASYNC, HEAVILY-MOCKED code — the pilot's four modules average one await and two are pure functions. A strengthening pass IS warranted, and it has a target. |

...
| A field with a fallback rule is read through that rule, everywhere. |  |  | `derived-fields.test.ts` |
| On the wizard's progress view, a trailing "…" marks an ACTION in progress — on either line. A line that names a STATE reached, or the THING being acted on, never takes one. |  |  | **—** |
| A modal does not change size while it works. |  |  | `modal-steady-height.test.ts` |
| A long operation reports where the SC is looking, and one surface does the narrating. Pressed on a screen, it opens that screen's progress modal; started anywhere else, it opens one notification; run by an agent, it goes to the agent's notifier. Work shorter than about ten seconds gets a busy state on the control that started it and nothing else. A blocking dialog is for a DECISION made before the work starts — never for progress, never for an outcome. A question the work cannot continue without — an expired sign-in, a missing prerequisite, a merge needing a decision — is asked through `askDuringOperation`, which sends it to whichever surface is already narrating. **A check the SC did not start asks nothing at all**: it reports what it found on the surface that wanted it. |  |  | `progress-surface.test.ts` |
...
| The cascade order is `reset < vendor < theme < overrides`, declared once and carried by every bundle. | [ADR](../architecture/adr/018-css-architecture.md) |  | `stylesheet-bundles.test.ts` |
| Every rule sits inside a cascade layer. None may sit outside one. | [ADR](../architecture/adr/018-css-architecture.md) |  | `stylesheet-bundles.test.ts` |
| Vendor CSS sits in the lowest cascade layer. | [ADR](../architecture/adr/018-css-architecture.md) |  | `stylesheet-bundles.test.ts` |
...
| Copy-paste in `src/` may not grow. The clone count is a shrink-only pin, and a fall must be banked. |  |  | `component-extraction.test.ts` |
| An abstract class has at least two implementations. |  |  | `component-extraction.test.ts` |
| No higher-order components — no `withX`, no `createXComponent`. |  |  | `component-extraction.test.ts` |
| A component generic over `<T>` earns it with size and real reuse. |  |  | `component-extraction.test.ts` |
...
| Every MCP tool declares which sign-ins it needs, and the declared providers are real ones. |  |  | `tool-auth-declarations.test.ts` |

...
- **[Evaluation Mode](./systems/evaluation-mode.md)** - MOVED to `feature/prompt-workbench` on 2026-08-26 (AI-3b). Only the consent dialog stayed; the Prompt Workbench, the trace view and the agent dry run did not. Kept as the record of what the surface was and why. Simulate a prompt with every change blocked and read what it would do, what it would cost, and where it wasted steps. Also the dry run that makes agent mutation impossible.

...
| 6e | Extend `sync_storefront` in `src/mcp-server.ts` to chain `previewAndPublishPage` (from `helixService.ts:910`) after successful `git push`, so AI edits propagate to the `.aem.page` preview URL without manual Republish. Mirrors the existing `configSyncService.syncConfigToRemote` pattern. Closes the AI edit → see-the-result loop. | Small-Medium | B |
| 6f | Replace `sync_storefront`'s ambient git auth with `GitHubTokenService`-injected credentials. Fetch a fresh token before `git push`; inject via `injectTokenIntoUrl` or `GIT_ASKPASS`. Eliminates silent push failures from expired or missing tokens. | Small | B |
...
| 6h | Fix `performAddonUpdates` in `src/features/updates/commands/updateExecutor.ts:287-330` to actually re-install block files when bumping `commitSha`. Today it advances the SHA without copying new files — a state lie. Call `installBlockCollections` (the same atomic-tree-commit install path used during project creation) before recording the new SHA. When the user has local edits to block files, switch from atomic overwrite to a 3-way merge: keep upstream content where the user hasn't edited, keep user's content where upstream hasn't changed, emit conflict markers where both have changed. Conflict resolution flow matches Step 6L — open VS Code Source Control + poll via `PollingService.pollUntilCondition` until clean. **Gated by `demoBuilder.blockLibraries.syncBehavior`** (see Section 7 "Configurable"): `enabled` applies automatically; `ask` prompts before copying files; `disabled` advances the recorded SHA without touching block files and surfaces a "Sync disabled" state in the Check for Updates UI. | Small-Medium | B |
...
| 6L | Service-first refactor: extract `storefrontSyncService.syncAndPublish(projectName, commitMessage)` in `src/features/eds/services/`. Vscode-free module; both the MCP and the extension import it. `sync_storefront` MCP tool becomes a thin wrapper. The service chains commit (if needed) + push (via `GitHubTokenService` auth) + Helix preview/publish (via `previewAndPublishPage`). Absorbs Steps 6e + 6f's intent into a single shared service. Non-technical-user conflict resolution: when `git push` is rejected (remote ahead), the service runs `git pull --rebase` automatically. If the rebase conflicts, the service emits conflict markers, opens VS Code's Source Control view via `vscode.commands.executeCommand('workbench.view.scm')`, then waits for resolution using the existing `PollingService.pollUntilCondition` (`src/core/shell/pollingService.ts`) — polls the conflicted files for absence of `<<<<<<<`/`=======`/`>>>>>>>` markers with exponential backoff. On user cancellation, the service runs `git rebase --abort` to leave the working tree exactly as it was. No "Open in Terminal" path; the entire flow is mouse-driven via VS Code's built-in merge editor. | Medium-Large | B |

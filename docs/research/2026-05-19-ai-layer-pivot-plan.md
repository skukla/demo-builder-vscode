...
**Stop writing `~/.claude/.mcp.json`**: the file is read by no consumer. Drop the write in `mcpConfigWriter.ts`. Existing instances on disk from prior installs are harmless dead data and are not auto-removed by Demo Builder — users can delete them manually if they want a clean home directory.
...
| 9 | ✅ Shipped Cycle C (2026-05-21). New `skillInspector` service walks `.claude/skills/`, parses YAML frontmatter via the same regex + `yaml.parse` shape as `skillsWriter::rewriteNameFrontmatter`, classifies skills as `'demo-builder'` (top-level lifecycle skills), `'adobe'` (nested under a bundle subdir), or `'unknown'` (other top-level). Returns `SkillInventoryEntry[]`. | Small | C |
| 10 | ✅ Shipped Cycle C (2026-05-21). New `mcpInspector` service reads `.claude/mcp.json`, spawns each server via `@modelcontextprotocol/sdk` (already a dependency — used server-side in `src/mcp-server.ts`), paginates through `tools/list`, returns `McpInventoryEntry[]` per server. 15s per-server timeout. Module-level `Map` cache with 5-minute TTL via `getCacheTTLWithJitter(CACHE_TTL.MEDIUM)`; only successful results cached. SDK env allowlist via `getDefaultEnvironment()` — host secrets do NOT leak to children. `clearMcpCache(serverId?)` exported for the `inspect-mcp` refresh path. | Medium | C |
| 11 | ✅ Shipped Cycle C (2026-05-21). New `sessionMcpDetector` reads `~/.claude.json::claudeAiMcpEverConnected` (top-level array — `mcp__claude_ai_*` is a runtime tool prefix, NOT a `mcpServers` key) cross-referenced with `~/.claude/mcp-needs-auth-cache.json`. Returns `SessionMcpEntry[]` with `{ displayName, needsAuth, lastSeen? }`. Best-effort: both files are undocumented Claude Code internal state. | Medium | C |
| 12 | ✅ Shipped Cycle C (2026-05-21). `verifyAiSetup` runs `gatherInventory` in parallel with the existing four checks and returns `inventory: AiInventory` alongside `checks`. Failures are isolated via `Promise.allSettled` and surface as `*Error` fields on the inventory payload. New `inspect-mcp` handler in `configureHandlers` clears the mcpInspector cache (per-server or all) then re-runs `inspectAllServers`. | Small | C |
...
- **Cycle B** — Steps 6a–6n + 7: install the App Builder MCP universally via `ai-defaults.json`, copy + namespace skill bundles per component via `aiSkillBundle` field on `components.json`, add type definitions, fix the consent-gated global MCP registration (`~/.claude.json` upsert), close the AI-edit → preview-URL sync loop by chaining Helix publish after `git push`, wire GitHubTokenService into the MCP's git auth, harden the PostToolUse hook (jq parser + path-with-spaces support + failure surfacing), fix the block library update bug, add multi-component repo enumeration in AGENTS.md, surface block origin libraries via `list_blocks`, add the read-only-sources note (6k), refactor sync logic into `storefrontSyncService` (vscode-free, shared between MCP and extension), add the `SyncStorefrontCommand` dashboard tile + command palette entry (6L/6m/6n), and add the `demoBuilder.ai.harness` setting. EDS projects gain Adobe's full skill agent set; future component additions ship their own bundles by declaration only. After Cycle B, the AI-edits-a-block-and-user-sees-it-on-preview loop closes end-to-end (matching the `te-eds-mock` 30-minute storefront experience) AND users have a non-AI sync surface for the same operation.
- **Cycle C** — Steps 9–12 (shipped 2026-05-21 on `feature/ai-layer-cycle-c`): skill and MCP inspection services + handler extensions. Backend data ready for the new tab.

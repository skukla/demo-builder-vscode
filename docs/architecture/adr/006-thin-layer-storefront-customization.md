...
| 7. Daily LKG drift-gate (GitHub Actions cron) | **Landed** | Multi-canonical gate in `skukla/eds-demo-patches/scripts/lkg-gate.sh`; per-ledger `canonical` + `lkgFile` fields; clones each unique canonical once per run; advances each LKG pointer independently. **Also carries the ADR-008 runtime-surface drift check** (`skukla/eds-demo-patches#1`) — advisory, riding the same per-ledger clone |
...
- **Mechanism precedents**: `src/features/eds/services/pdp/pdp404HandlerPublisher.ts` (code vendoring, ADR-005), `src/features/eds/services/brandAssetPublisher.ts` (brand-asset vendoring — the "1 vendor point" this ADR prescribed; added 2026-08-14), `src/features/eds/services/patches/contentPatchRegistry.ts` (`contentPatches` + external `ContentPatchSource`), `src/features/eds/services/blockCollectionHelpers.ts` (block install + UE merge)

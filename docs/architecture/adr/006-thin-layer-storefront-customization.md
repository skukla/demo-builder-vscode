...
**Live on develop.** Four demo packages now drive the thin-layer pipeline — CitiSignal, `custom`, `b2b`, and `bodea` (added 2026-08-14, hidden; shares the b2b ledger + LKG pin, with its brand delta shipped additively via the bodea-blocks library and the brand-assets vendor point). The mechanism handles multi-canonical patches repos (the gate at `skukla/eds-demo-patches` clones one canonical for citisignal+custom and a different one for b2b, advancing per-ledger LKG pointers independently). The earlier "dormant until Step 5b" framing is now historical.

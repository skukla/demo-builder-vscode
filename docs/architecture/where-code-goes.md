...
| 9 | Add feature configuration | JSON in the feature's `config/` | **BUNDLED config — the normal case:** a static `import cfg from '../config/x.json'`, which esbuild inlines. Give the reader an injection seam (`getPackageById(id, packages = bundled)`) instead of mocking the module. **DISK config — rare:** `ConfigurationLoader`, only when the path is resolved at runtime (`prerequisites.json`, the component registry) | `no-config-leaf-mocks` enforces the seam (do not MOCK a leaf). Nothing checks WHICH mechanism you picked |

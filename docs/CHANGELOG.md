...
- **"You hold no admin role on this site" now tells you how to fix it yourself.** Every message about this refusal used to end by naming someone else to ask — and when nobody was visible, it had nothing to offer. Reinstalling the AEM Code Sync GitHub App grants the role: set your Adobe address as your primary email on GitHub, uninstall the app completely, then install it again and re-grant every repository. Removing and re-adding a single repository reports the same success and does **not** grant it, which is what made this look unfixable. The instruction is now the same on all four screens that report the refusal, and Manage Site Access opens the GitHub page where the app is uninstalled.
- **Manage Site Access warns when your GitHub primary email would send the role to the wrong address.** The role is granted to whichever address is primary when AEM Code Sync is installed, so reinstalling with the wrong one grants it to the wrong one. When your primary email is not the address Demo Builder signs in to Adobe with, it now says so, in that order, and offers the GitHub page where you change it.

...
### Added
- **The ERP integration, the first pre-built integration in the catalog.** One tile in the Integrations gallery gives a project two things: an ERP modelled on SAP with its own screen, and an Adobe Commerce integration to it built on the Commerce integration starter kit. Adding the tile deploys the ERP first, then the integration; the ERP mirrors the instance's products and companies; orders get an ERP number, ERP price, stock and status changes land in Commerce, contract prices apply in the cart, and a screen inside the Commerce Admin shows it. The ERP gets its own card beside the integration's, with an "ERP" badge, its own status, "Open", "Reset records" and "Redeploy", and each card names the other; the screen counts them separately ("1 integration · 1 system") and the dashboard's Integrations tile includes the ERP's health. Removing either card removes both. First the integration undoes the credit limits, company blocks and ERP order numbers it set in Commerce, it is uninstalled from Commerce, and the ERP's records are deleted; only then are the two undeployed. If one of those steps fails, nothing is removed: the card reads "Removal stopped", says why, and offers "Remove anyway", which goes ahead and names what stays behind. Agents get the same choice through `remove_integration` with `force`. Agents get `get_erp_status` and `reset_erp_records`. Live acceptance on a Commerce instance is still to run.
- **Updating an installed integration.** Opening the integrations screen checks whether each deployed integration, and the ERP, has newer code on its GitHub branch or a version Commerce has not installed; a card that has reads "Update needed", and so does the dashboard's Integrations tile. Update now fetches the newer code (fast-forward only; it refuses and names the files when the folder holds your edits), installs its dependencies and redeploys, which upgrades the app in Commerce. It used to redeploy the folder unchanged. Agents get `update_integration` and `check_integration_updates`.
- **Reinstall in Commerce, when an upgrade is refused.** When Commerce will not upgrade an installed App Management integration in place, the integrations screen opens a confirmation offering to reinstall it: remove it from Commerce, then install the version already deployed. Until it is done, the card's menu offers "Reinstall in Commerce"; it appears nowhere else. Agents get `reinstall_integration`, which is confirm-gated and refused unless Commerce refused an upgrade.
...
### Fixed
- **An update is no longer refused because of a file the extension itself rewrote.** Deploying a plain App Builder app — the ERP that comes with the ERP integration — rewrites its `app.config.yaml` to keep its Runtime packages apart, which re-saves the file and drops its comments. The next update then reported "The integration folder has changes of its own (app.config.yaml)" and stopped, blaming an edit nobody made. That file now counts as the extension's, like `package-lock.json`: it is put back before the update and written again by the deploy that follows.
...
## [1.0.0-beta.147] - 2026-09-23

### Fixed
- **The extension package is small again.** beta.146 shipped at 41.2 MB against beta.145's 7.7 MB: the release-cut sweep copies the whole repository into a sandbox to mutate it, and the packaging step minutes later swept that copy into the VSIX — 8,836 files that are not the extension. The sandbox directory is now excluded from packaging.
- **"You hold no admin role on this site" now tells you how to fix it yourself.** Every message about this refusal used to point at AEM's setup page, which can only grant anything when AEM Code Sync opens it during a GitHub App install — opened from Demo Builder it could not load the site or add anyone. That link is gone, and in its place is a fix you can do: set your Adobe address as your primary email on GitHub, uninstall the AEM Code Sync app completely, then install it again and re-grant every repository. Removing and re-adding a single repository reports the same success and does **not** grant the role, which is what made this look unfixable. The instruction is identical on all four screens that report the refusal, and Manage Site Access now opens the GitHub page where the app is uninstalled. An agent whose repair is refused gets the same hand-off.
- **Manage Site Access warns when your GitHub primary email would send the role to the wrong address.** The role goes to whichever address is primary when AEM Code Sync is installed, so reinstalling with the wrong one grants it to the wrong one. When your primary email is not the address Demo Builder signs in to Adobe with, it says so — before the reinstall, not after — and offers the GitHub page where you change it.

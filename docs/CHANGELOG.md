...
- **`Demo Builder: Open in Claude Code` command** (Cycle D, Step 14): Launches Claude Code on the current project. URI handler (`vscode://anthropic.claude-code/open`) when the extension is installed and `harness !== 'terminal'`; otherwise opens a terminal in the project directory and sends `claude`. First successful URI launch shows a one-time tip about dragging the Claude Code panel to the secondary side bar (tracked via `globalState.demoBuilder.ai.firstClaudeOpenTipShown`).
...
### Changed
- **`demoBuilder.ai.harness` setting replaced with `demoBuilder.ai.engine` + `demoBuilder.ai.surface`** (supersedes the Cycle B addition above). The 3-mode `auto` / `extension` / `terminal` setting collapses to `engine='claude-code'` (reserved for future engines such as Codex) + `surface='extension'` / `'terminal'`. The opaque `auto` mode is removed; default is `surface='extension'`. A one-time migration in `extension.ts:migrateHarnessSetting` maps `auto` → `extension` per scope (workspace + global + workspaceFolder handled independently); an explicit `surface` choice at any scope takes precedence. Missing-extension now surfaces a recovery dialog (Install Claude Code Extension / Switch to Terminal Mode) instead of a silent error or fallback. A first-launch setup dialog (`maybeShowFirstLaunchDialog`) fires once when `surface='extension'` + extension not installed. Terminal mode runs `claude --continue` (was `claude`) and find-or-spawns the "Claude Code" terminal per project per window. Prompt clicks from the AI surface anchor the workspace via `PENDING_CLAUDE_LAUNCH_KEY` and a replay handler in `extension.ts:activate` so the chat panel opens with full project context regardless of starting workspace. See ADR-004 Amendment (2026-05-24b).

...
117 tools, grouped by the module that registers them.
...
| Tool | | What it does |
|---|---|---|
| `add_shared_demo` | **confirm** | Add a colleague's demo (or one of your own) to the Add a demo list from its GitHub link or site address, so create_project can build on it. keepCopy (default true) forks the repository into your own G |
...
| `forget_added_demo` | **confirm** | Take a demo added from a link off the Add a demo list; with deleteCopy:true also delete your own copy of its code from GitHub. Projects built on it are never touched. Requires confirm:true. |

...
141 tools, grouped by the module that registers them.
...
| `check_setup_steps` |  | Run the checks Demo Builder can do itself on an integration's demo setup checklist |
...
| `set_setup_step` |  | Mark one step of an integration's demo setup checklist (from get_setup_checklist) |
...
| `write_erp_rest` | **confirm** | POST, PUT, PATCH or DELETE one of the ERP's own routes — the actions a person takes |
...
| `get_setup_checklist` | read | Read an integration's demo setup checklist: what the SC prepares by hand in Commerce |

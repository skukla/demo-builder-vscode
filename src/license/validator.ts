...
export class LicenseValidator {
    ...
    private encrypt(text: string): string {
        // Use a fixed IV for backward compatibility (not ideal for security, but maintains existing data)
        const iv = Buffer.alloc(16, 0); // 16 bytes of zeros
        const key = crypto.scryptSync(LicenseValidator.ENCRYPTION_KEY, 'salt', 32);

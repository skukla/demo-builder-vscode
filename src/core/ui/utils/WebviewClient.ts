...
interface PendingRequest {
    ...
    timeout: ReturnType<typeof setTimeout>; // Handle both browser number and Node Timeout
}
...
class WebviewClient {
    ...
    private getApi(): VSCodeApi {
        ...
        if (!this.vscodeApi) {
            throw new Error('WebviewClient: Failed to initialize VS Code API');
        }
    ...
    public onMessage(type: string, handler: (data: unknown) => void): () => void {
        ...
        const handlers = this.listeners.get(type) ?? new Set<(data: unknown) => void>();

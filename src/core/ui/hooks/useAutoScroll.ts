...
export function useAutoScroll<
    C extends HTMLElement = HTMLDivElement,
    I extends HTMLElement = HTMLDivElement,
>(options: UseAutoScrollOptions = {}): UseAutoScrollReturn<C, I> {
    const {
        enabled = true,
        behavior = 'smooth',
    ...
    const scrollToItem = (index: number) => {
        ...
        setTimeout(() => {
            ...
            // An item that is already fully visible satisfies neither branch below, so
            // the two branches ARE the "only scroll if not visible" rule.

...
<body>
    ...
    protected initializeMessageHandlers(comm: WebviewCommunicationManager): void {
        ...
        comm.on('save-configuration', async (data: { componentConfigs: any }) => {
            try {
                ...
                this.logger.info('[Configure] Received componentConfigs:', JSON.stringify(data.componentConfigs, null, 2));
    ...
    private async generateComponentEnvFile(
        ...
        this.logger.info(`[Configure] Writing ${componentId} config to ${envPath}`);
        this.logger.info(`[Configure] Config for ${componentId}:`, JSON.stringify(config, null, 2));
        ...
        this.logger.info(`[Configure] Writing ${lines.length} lines to ${envPath}`);
        await fs.writeFile(envPath, lines.join('\n'), 'utf-8');
        this.logger.info(`[Configure] Successfully wrote ${envFileName} for ${componentId}`);
    }
}

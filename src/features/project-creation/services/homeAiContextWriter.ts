...
function buildHomeWorkingOnProjects(): string {
    return [
        ...
        '`deploy_mesh`, etc.).',
        '',
        '**When the user refers to a project without naming one, call the `get_current_project`',
        'MCP tool to resolve the active project; ask which one they mean only if it returns null.**',
        '',

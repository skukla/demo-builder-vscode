...
3. **Map tokens to drop-in CSS custom properties.** Each drop-in publishes a CSS variable surface. Update the project's storefront CSS to set these variables to the scraped values. Reuse the existing `aem-block-developer` skill for the actual CSS writing.
   ...
   **Typography maps to the shipped scale, not to raw pixel values.** The storefront already defines a complete type scale as `--type-*` custom properties in `styles/styles.css` (display, headline, body, details — each a full `font` shorthand). Express the scraped type scale by ADJUSTING those properties' values, and style block text with `font: var(--type-…)` — never write a literal `font-size` into block CSS. One scale edit then retunes every block consistently; scattered literals are why "the fonts are too small" cannot be fixed in one place.

   **The same rule holds for the WHOLE theme, not just type.** `styles/styles.css` defines the storefront's entire theme as custom-property tokens — colors (`--color-*`), spacing (`--spacing-*`), corner/border treatment (`--shape-*`), and the layout grid (`--grid-*`); the shipped template carries over a hundred of them. Read that file FIRST to learn the real inventory (it is the source of truth — templates evolve, so never work from a remembered list), then express every scraped value the token system covers by adjusting tokens and referencing them with `var(--…)` in block CSS. Theming is a bounded edit of one file, not an unbounded search across block styles — a literal hex or pixel value in block CSS is only correct when no token covers that property.

   **Say the lifecycle out loud before a big theming pass: a project RESET returns the storefront repo to template state, destroying every CSS edit — tokens included — and re-vendors brand files from their source repo.** That is what reset is for, but nothing warns the person asking. If the theme you are building must survive resets, it belongs upstream (the demo package's brand source repo), which is a change to the shared package — ask the user before touching it.

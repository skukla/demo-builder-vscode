...
- A browser: Playwright uses the machine's installed Google Chrome by default (no download). Only Chrome-less machines need the one-time ~150 MB Chromium install (`install-browser` subcommand; shared via `~/Library/Caches/ms-playwright/`).

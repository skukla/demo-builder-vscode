...
|---|---|---|
| **Brand identity** | Wrong primary color, wrong font, wrong logo placement | HIGH — fix in round 1, via the `--color-*` / `--type-*` tokens in `styles/styles.css`, never per-block literals |

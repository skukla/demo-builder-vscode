...
import { dataInstallerHandlers } from '../handlers';
...
export class ShowDataInstallerCommand extends BaseWebviewCommand {
    ...
    protected initializeMessageHandlers(comm: WebviewCommunicationManager): void {
        // The whole map in one loop — never a per-message list. A type that is in
        // the map but not registered here would hang its request silently.
        for (const messageType of getRegisteredTypes(dataInstallerHandlers)) {

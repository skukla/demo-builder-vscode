...
- `skillsWriter.ts` - Writes thirteen always-on Demo-Builder skills to `.claude/skills/`: four lifecycle (add-component, sync-changes, update-credentials, create-eds-project) plus diagnose-demo (the only one that tells an agent how to LOOK rather than how to DO — routes a symptom to the check that answers it) plus six EDS site-scraping skills (scrape-reference-site, connect-authenticated-site, commerce-block-mapper, demo-data-injector, header-nav-footer, refine-visual-match) plus two block-library registration skills (register-custom-block, remove-custom-block); Projects that BUILD an App Builder app — a mesh or an attached component, NOT a bare EDS storefront (`projectBuildsAppBuilderApps` in `aiToolingGate`; entries flagged `thirdParty` — Playwright — are additionally gated on the `demoBuilder.ai.enableThirdPartyTools` setting through the gate's injected resolver, and their dependent skills gate with them) also get extend-app-builder-app (the list_console_apis → add_console_apis loop) and the `commerce-extensibility-tools` integration-starter-kit bundle (`appbuilder-*` prefix, from the isolated tools dir); EDS projects additionally get the `aem-boilerplate-commerce` bundle (`aem-*` prefix, six skills — block-developer, content-modeler, dropin-developer, project-manager, researcher, tester) from that SAME isolated tools dir: Adobe ships every starter-kit bundle inside one package, so neither bundle is sourced from a component's own checkout

...
- `aiContextWriter.ts` - Generates `AGENTS.md` at the project root with project-specific AI agent context; writes `CLAUDE.md` (root) and `.claude/CLAUDE.md` as one-line `see @AGENTS.md` pointers. The section content lives in `agentsMdSections.ts` — including the "Finding Adobe Documentation" section routing agents to Adobe's Wayfinder doc router, PINNED to a commit (`WAYFINDER_ROUTER_URL`) — never `@main`, since that line becomes part of the agent's instructions in a user's repo. Re-pinning is a bundle change: bump `AI_CONTEXT_VERSION`. The "Querying Commerce" section (v23) names `get_commerce_endpoints` and warns that a wrong-scope Catalog Service query returns an EMPTY result with no error; it points at the TOOL rather than baking endpoint values in, because those change between regenerations and a confidently stale endpoint is worse than none.
- `mcpConfigWriter.ts` - Generates `.claude/mcp.json`, `.mcp.json`, and `.claude/settings.json` (Cursor and Codex read `.mcp.json` natively — no per-tool config files); the settings/hook builders live in `claudeSettingsWriter.ts` (PostToolUse git-sync + PreToolUse aio-global guard)
- `skillsWriter.ts` - Writes thirteen always-on Demo-Builder skills to `.claude/skills/<name>/SKILL.md` (directory layout — the only shape Claude Code registers as invocable skills; flat `<name>.md` until v27): four lifecycle (add-component, sync-changes, update-credentials, create-eds-project) plus diagnose-demo (the only one that tells an agent how to LOOK rather than how to DO — routes a symptom to the check that answers it) plus six EDS site-scraping skills (scrape-reference-site, connect-authenticated-site, commerce-block-mapper, demo-data-injector, header-nav-footer, refine-visual-match) plus two block-library registration skills (register-custom-block, remove-custom-block); Projects that BUILD an App Builder app — a mesh or an attached component, NOT a bare EDS storefront (`projectBuildsAppBuilderApps` in `aiToolingGate`; entries flagged `thirdParty` — Playwright — are additionally gated on the `demoBuilder.ai.enableThirdPartyTools` setting through the gate's injected resolver, and their dependent skills gate with them) also get extend-app-builder-app (the list_console_apis → add_console_apis loop) and the `commerce-extensibility-tools` integration-starter-kit bundle (`appbuilder-*` prefix, from the isolated tools dir); EDS projects additionally get the `aem-boilerplate-commerce` bundle (`aem-*` prefix, six skills — block-developer, content-modeler, dropin-developer, project-manager, researcher, tester) from that SAME isolated tools dir: Adobe ships every starter-kit bundle inside one package, so neither bundle is sourced from a component's own checkout

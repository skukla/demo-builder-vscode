...
The **MCP server** is compiled from `src/mcp-server.ts` (not from inside this feature directory) to `dist/mcp-server.js`. It runs as a standalone stdio process — no VS Code dependency — and exposes eight tools to AI agents:

 ...
 * {@link DeployableRunnerDeps} so the runner is pure orchestration. The production
 * defaults wire the real functions; unit tests mock them.
 */

import { setDeployable, getProvidedEnvVars } from './deployableState';
 ...
 */
export interface DeployableRunnerDeps {
    ...
    subscribeRequiredApis: (deployables: DeployableCatalogEntry[], project: Project) => Promise<void>;
    ...
    /** Every deployable in the project's catalog (for the union subscribe). */
...
function targetFor(project: Project, deps: DeployableRunnerDeps) {
...
function buildDefinition(entry: DeployableCatalogEntry): TransformedComponentDefinition {
...
function findMissingProvider(project: Project, entry: DeployableCatalogEntry): string | undefined {
    for (const envVar of entry.envSchema ?? []) {
        const provider = envVar.providedBy;
        if (provider && !project.deployables?.[provider]) {
            return provider;
        }
    }
    return undefined;
...
/** Persist `deployables[id]` and save; returns the updated project. */
async function persistResult(
    ...
    const updated = setDeployable(project, id, state);
    await deps.saveProject(updated);
    return updated;
}

/** Republish the storefront when the project carries provided env vars (else no-op). */
async function republishIfProvided(project: Project, deps: DeployableRunnerDeps): Promise<void> {
...
/** Build the persisted DeployableState from a successful mesh deploy. */
function meshState(entry: DeployableCatalogEntry, data: MeshDeploymentResult['data']): DeployableState {
...
/** Build the persisted DeployableState from a successful integration deploy. */
function integrationState(entry: DeployableCatalogEntry, data: AppDeploymentResult['data']): DeployableState {
    return {
        kind: 'integration',
        status: 'deployed',
        source: { owner: entry.source.owner, repo: entry.source.repo, branch: entry.source.branch },
...
function errorState(entry: DeployableCatalogEntry): DeployableState {
    return {
        kind: entry.kind,
        status: 'error',
        source: { owner: entry.source.owner, repo: entry.source.repo, branch: entry.source.branch },
...
async function dispatchDeploy(
    ...
    deps: DeployableRunnerDeps,
): Promise<{ ok: true; state: DeployableState } | { ok: false; error: string }> {
    if (entry.kind === 'mesh') {
...
/**
 * Add a deployable: subscribe → clone+install → kind-dispatched deploy (under
 ...
 */
export async function addDeployable(
    project: Project,
    entry: DeployableCatalogEntry,
    deps: DeployableRunnerDeps,
    ...
    try {
        ...
        deps.logger.error('[Deployable Runner] add failed', error as Error);
        return { success: false, error: toError(error).message };
    }
}

/**
 * Redeploy ONLY the given deployable's tail (no re-clone), under org-context.
 * Touches only its own entry.
 */
export async function deployDeployable(
    project: Project,
    id: string,
    deps: DeployableRunnerDeps,
): Promise<RunnerResult> {
    const existing = project.deployables?.[id];
    const componentPath = project.componentInstances?.[id]?.path;
    if (!existing || !componentPath) {
        return { success: false, error: `Deployable "${id}" not found.` };
    }
    ...
    try {
        ...
        deps.logger.error('[Deployable Runner] deploy failed', error as Error);
        return { success: false, error: toError(error).message };
    }
}

/** Reconstruct a minimal catalog entry from persisted state (redeploy fallback). */
function entryFromState(id: string, state: DeployableState): DeployableCatalogEntry {
    return {
        id,
        name: id,
        description: '',
...
/** Tear down the remote artifact for a deployable, by kind, under org-context. */
...
/**
 * Remove a deployable: kind-dispatched remote teardown (best-effort) → delete the
 * local folder → clear `deployables[id]` → if it provided env vars, regenerate the
 * storefront config WITHOUT them.
 */
export async function removeDeployable(
    project: Project,
    id: string,
    deps: DeployableRunnerDeps,
): Promise<RunnerResult> {
    const state = project.deployables?.[id];
    if (!state) {
        return { success: false, error: `Deployable "${id}" not found.` };
    }
    ...
    try {
        ...
        deps.logger.warn(`[Deployable Runner] remote teardown warning: ${toError(error).message}`);
    }

    await deps.componentManager.removeComponent(project, id, true);

    const cleared = { ...project, deployables: { ...(project.deployables ?? {}) } };
    delete cleared.deployables[id];

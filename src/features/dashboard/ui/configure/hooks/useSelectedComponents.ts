...
export interface SelectedComponent {
    id: string;
    data: TransformedComponentDefinition;
    type: string;
}
...
export function useSelectedComponents({
    ...
    return useMemo(() => {
        ...
        const addComponentWithDeps = (comp: TransformedComponentDefinition, type: string) => {
        ...
        if (project.componentSelections?.frontend) {
            const frontend = componentsData.frontends?.find(
                (f) => f.id === project.componentSelections?.frontend,
            );
            if (frontend) addComponentWithDeps(frontend, 'Frontend');
        }

        if (project.componentSelections?.backend) {
            const backend = componentsData.backends?.find(
                (b) => b.id === project.componentSelections?.backend,
            );
        ...
        // No appBuilder branch: the wire has no `componentsData.appBuilder`
        // bucket since c98e5125 removed the appBuilderApps mechanism. App
        // Builder env fields flow via `appBuilderComponentCatalog` (bucket 3),
        // so resurrecting the bucket here would double-render them.

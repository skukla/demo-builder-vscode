/**
 * Deployable Handlers (D2 Track B — Step 05)
 *
 * The dashboard message handlers that drive the live D1 deploy-contract runner
 * from the integrations list. THIS is the first UI-driven `addDeployable`
 ...
 * FIRST, so a deployable that needs user inputs is never silently deployed with
 * missing values.
 *
 ...
 *
 * @module features/dashboard/handlers/deployableHandlers
 */
...
} from '@/features/app-builder/services/deployableRunnerDeps';
import {
...
function resolveCatalog(project: Project): DeployableCatalogEntry[] {
...
async function runGuards(context: HandlerContext, project: Project): Promise<string | undefined> {
    ...
    const authResult = await ensureAdobeIOAuth({
        authManager,
        logger: context.logger,
        logPrefix: '[Deployables]',
        projectContext: {
            organization: project.adobe?.organization,
            projectId: project.adobe?.projectId,
            workspace: project.adobe?.workspace,
        },
        warningMessage: 'Adobe sign-in required to manage deployables.',
    });
...
function customEntry(source: { owner: string; repo: string }): DeployableCatalogEntry {
    return {
        id: `${source.owner}-${source.repo}`,
        name: source.repo,
        description: `Custom deployable from ${source.owner}/${source.repo}`,
        kind: 'integration',
...
async function postRowStatus(
    ...
    await ProjectDashboardWebviewCommand.sendDeployableStatusUpdate(id, status, message);
}

/**
 * Handle 'addDeployable' — guards → (bucket-3 → Configure) → assemble deps →
 * D1 addDeployable. The FIRST live UI-driven full add.
 */
export const handleAddDeployable: MessageHandler<{
    id?: string;
    ...
    if (!entry) {
        return { success: false, error: 'Unknown deployable', code: ErrorCode.CONFIG_INVALID };
    }
    ...
    await postRowStatus(entry.id, 'deploying', 'Adding deployable…');
    const deps = buildDefaultRunnerDeps(await buildRunnerDepsContext(context, project));
    const result = await addDeployable(project, entry, deps);
    if (!result.success) {
...
/** Shared deploy/redeploy: guards → D1 deployDeployable {id}. */
async function deployById(context: HandlerContext, id: string | undefined) {
    if (!id) {
        return { success: false, error: 'Deployable id is required', code: ErrorCode.CONFIG_INVALID };
    }
    const project = await context.stateManager.getCurrentProject();
    if (!project) {
        return { success: false, error: 'No project found', code: ErrorCode.PROJECT_NOT_FOUND };
    ...
    const deps = buildDefaultRunnerDeps(await buildRunnerDepsContext(context, project));
    const result = await deployDeployable(project, id, deps);
...
/** Handle 'deployDeployable' — deploy the given deployable's tail. */
export const handleDeployDeployable: MessageHandler<{ id?: string }> = (context, payload) =>
    deployById(context, payload?.id);

/** Redeploy is the same path (idempotent re-run of the deploy tail). */
export const handleRedeployDeployable = handleDeployDeployable;

/** Handle 'removeDeployable' — guards → D1 removeDeployable {id} (confirm is UI-side). */
export const handleRemoveDeployable: MessageHandler<{ id?: string }> = async (context, payload) => {
    const id = payload?.id;
    if (!id) {
        return { success: false, error: 'Deployable id is required', code: ErrorCode.CONFIG_INVALID };
    }
    ...
    const deps = buildDefaultRunnerDeps(await buildRunnerDepsContext(context, project));
    const result = await removeDeployable(project, id, deps);
    return result.success ? { success: true } : { success: false, error: result.error };
};

/**
 * Handle 'verifyDeployable' — an ON-DEMAND, non-interactive probe (P1). It uses
 ...
 */
export const handleVerifyDeployable: MessageHandler<{ id?: string }> = async (context, payload) => {
    const id = payload?.id;
    if (!id) {
        return { success: false, error: 'Deployable id is required', code: ErrorCode.CONFIG_INVALID };
    }

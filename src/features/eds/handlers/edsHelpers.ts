...
export async function bulkPreviewAndPublish(
    ...
    // Preview and publish report separately. One catch around both labelled every
    // failure "Bulk publish failed", so a preview 401 sent the reader looking at
    // the publish step.
    try {
        await helixService.previewAllContent(owner, repo, 'main', undefined, paths);
    } catch (error) {
        logger.debug(`[EDS] Bulk preview failed: ${(error as Error).message}`);
        throw error;
    }
    try {

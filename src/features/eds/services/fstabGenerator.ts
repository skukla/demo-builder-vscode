 ...
 * {@link generateFstabContent}, matching the one line it writes. A repository
 * whose fstab mounts something else (a SharePoint or Google Drive source, a
 * BYOM markup service) has no DA.live content site to copy, and answers
 * `undefined`.
 *
...
export function parseFstabContentSource(text: string): { org: string; site: string } | undefined {
    const match = /^\s*\/:\s*https:\/\/content\.da\.live\/([^/\s]+)\/([^/\s]+)\/?\s*$/m.exec(text);

...
import { ComponentEnvVar, ComponentConfigs, WizardState, WizardStep } from '@/core/ui/types';
...
export function ComponentConfigStep({ state, updateState, setCanProceed }: ComponentConfigStepProps) {
    ...
    // Load components data using request-response pattern
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        selectedComponents.forEach(({ id, data }) => {
            if (!newConfigs[id]) {
                ...
                // Set defaults from required env vars
                data.configuration?.requiredEnvVars?.forEach((envVarKey: string) => {
                    const envVarDef = envVarDefs[envVarKey];
                    if (envVarDef?.default !== undefined) {
                        newConfigs[id][envVarKey] = envVarDef.default;
                    }
                });

                // Set defaults from optional env vars
                data.configuration?.optionalEnvVars?.forEach((envVarKey: string) => {
                    const envVarDef = envVarDefs[envVarKey];
                    if (envVarDef?.default !== undefined) {
                        newConfigs[id][envVarKey] = envVarDef.default;
                    }
